...
1. **配置单元 ≠ 模拟单元 ≠ 统计单元**：终末地的「限定寻访」对玩家而言是**一个卡池**，但当前方案要求用户手动创建 3 个 `[[pools]]`（主池 + 送抽池 + 档案池）并手动连线。统计时3行独立的 pool 数据需手动聚合。
...
| 配置泄漏实现细节 | 用户手动创建3个 `[[pools]]` + `InterruptConfig` 连线 | Pool 是物理单位，用户需要的是逻辑单位 |
...
- [x] 配置一次性迁移：`[[pools]]` → `[[banner]]`（DECISION-3/4，无历史包袱），解析器只认 `[[banner]]`、无自动包装
...
class Pool:
    ...
    max_draws: Optional[int] = None  # 该 pool 的最大抽取次数（由引擎自动执行——pool 抽数达上限后自动标记 exhausted，无需手写 lifecycle 规则）
                                     ...
                                     # None=无上限（REVIEW-R1-FIX: ISSUE-331：TOML/UI 层「0=无限制」在解析/保存边界
                                     #   归一化为 None——max_draws=0 直接落引擎则 _pool_draws >= 0 恒 True、首抽即 exhausted）
...
class TransitionPreview:
    ...
    remaining: int                   # 还差多少（阈值型语义；事件型为 -1，REVIEW-R1-FIX: ISSUE-305）：
                                     ...
                                     #   time_window：max(0, ceil((at_value - 当前时间) / DAY)) 向上取整剩余天数
                                     #     ——at_value 与当前时间均为【秒】（ISSUE-001），先经 /DAY 换算为天再向上
                                     #       取整（不截断小数——2.5 天→3 而非 2，避免近边界垫刀 1 天误差）；
                                     #       若直接 ceil(at_value - 当前时间) 会得到秒而非天、与垫刀判据
                                     #       差 86400 倍（REVIEW-R1-FIX: ISSUE-317——单位口径与 ISSUE-001 秒统一）；
                                     #       「当前时间」由调用方经 pending_transitions(real_time) 入参提供
                                     #       （Banner 不落库 real_time，与 is_available(real_time) 同源——
                                     #       REVIEW-R1-FIX: ISSUE-330）
...
class DrawOutcome:
    ...
    pity_triggered: bool  # 本次是否触发保底（featured_ids 判定，等价现状 gacha_service L305-316）。
                          # **excludes_all_pity 池恒为 False（REVIEW-R1-FIX: ISSUE-320）**——该类池完全旁路
                          # before_draw/after_draw、不触碰 pity_state（§3.5 要点 4），本判定分支不查询
                          # spec/featured_ids（不调用 get_spec），保证 BannerStats.pity_triggers / collector
                          # 的 pity_triggers 统计与 §3.8「仅统计非 excludes_all_pity 的 pool」一致，
                          # 否则 featured 卡在旁路池仍令 pity_triggered=True、stats 自增（冲突）
...
class Banner:
    ...
    max_draws: Optional[int] = None                 # Banner 级总抽数硬上限——由引擎自动执行（与 Pool.max_draws 对称，
                                                     ...
                                                     #   （after_draw 订阅）」不冲突；None=无上限（REVIEW-R1-FIX: ISSUE-331：
                                                     #   TOML/UI 层「0=无限制」在 _build_banners 解析边界与 UI 保存边界
                                                     #   归一化为 None——max_draws=0 直接落引擎则 `_total_draws >= 0`
                                                     #   恒 True、首抽即自动 exhaust，与「0=无限制」语义相反）
    ...
    def is_available(self, real_time: float) -> bool:
        """可用性判定——**必须显式传入当前模拟时间 real_time（秒，§3.2 时间单位规范）**。
        时间求值输入（REVIEW-R1-FIX: ISSUE-001）：Banner 不落库 real_time（本类注释「无 card_id /
        real_time 落库」），时间窗口（available_from / available_until）与实时时间的比较无法在
        无参 property 内完成——改为带参方法，由调用方（gacha_service 过滤处、策略）传当前
        `state.real_time`。定义：
        ...
        ——「可用性完全由 Banner 级 available_from/until + lifecycle 激活状态决定」（§3.13.4）的
        引擎落点：窗口未开（real_time < available_from）与已关（real_time > available_until）的
        Banner 均返回 False、不进入 `active_banners`（§3.5 过滤处传 `state.real_time`），策略不可
        再抽——与现状 gacha_service.py:224-226 逐池 real_time 过滤语义等价。生命周期
        `exhaust_banner` 动作经 `_exhaust()` 置 `is_exhausted=True` 亦使本判定为 False。
        时间窗口过期由「is_available 时间比较」+「on_banner_end 关闭标记」双路径生效（§3.5 要点 6）。"""
    @property
    ...
    def pending_transitions(self, real_time: float) -> List[TransitionPreview]:
        """带参方法（REVIEW-R1-FIX: ISSUE-330）——与 is_available(real_time) 对称：Banner 不落库
        real_time，time_window 条件的 remaining（`max(0, ceil((at_value - 当前时间) / DAY))`，
        见 TransitionPreview 注释）与 current_value 需要当前模拟时间——由调用方（策略）传
        `state.real_time`（策略经 ctx.state.real_time 获取，§3.7 示例 `for pt in
        banner.pending_transitions(ctx.state.real_time)` 已同步修订）。"""
    ...
    def draw(self, state, pity_engine, pity_state, pool: Optional["Pool"] = None) -> "DrawOutcome":
        ...
        1) excludes_all_pity → 旁路 before_draw/after_draw，不触碰 pity_state（保底旁路），
           且 DrawOutcome.pity_triggered 恒 False、triggered_pity_name 恒 None——不查询
           spec/featured_ids（REVIEW-R1-FIX: ISSUE-320，见 DrawOutcome 注释）
        ...
        3) 计算 pity_triggered + triggered_pity_name（featured_ids 判定 + spec.pity_names 拼接，等价现状
           gacha_service L305-316；excludes_all_pity 池跳过本步），携带实际产出 pool_id 返回 DrawOutcome
...
> **兜底语义（活跃池耗尽，REVIEW-R1-FIX: ISSUE-010）**：单 Banner 内活跃池因 `max_draws` 触发 exhausted 后（一次性池 = max_draws=batch_size），若无 `pool_exhausted` lifecycle 规则接管（允许只配置一个池），Banner **自动 exhaust**（等价 `exhaust_banner` 兜底）——`is_available(real_time)=False`、策略侧 `is_exhausted=True`、`pending_transitions(real_time)` 无挂起项（带参方法，REVIEW-R1-FIX: ISSUE-330），draw() 不再被调用。避免路由到已耗尽池产生未定义行为。该语义进入 `_exhaust()` 实现。
>
...
- 现有字段保留：`id` / `name` / `cost` / `rewards` / `batch_size` / `epitomizable_cards` / **`exchange_card_id`（REVIEW-R1-FIX: ISSUE-313——smart.py:48 / pity_reserve.py:36 / pool_quota.py:38 / stop_on_target.py:47 均以 `pool.is_exchange and pool.exchange_card_id == t.card_id` 定位兑换池；该字段若按保留清单之外的『未列即删』处置则 4 策略原子提交即 AttributeError。显式列入保留清单：运行时 `Pool.exchange_card_id` 保留，Ph6 从 `BannerPoolEntry.exchange_card_id`（§3.9 已有字段）透传构造，Ph9 兑换池回归锁定）**
...
- **迁移方案（新增 Ph1a，见 §3.12）**——8 个内置策略（smart / pity_reserve / pool_quota / stop_on_target / draw_target / no_draw / fixed_count / target_hunting）统一以 `ctx.banners` 为读取源，逐项替换：
  - `pool.is_available_at(real_time)` → `banner.is_available(ctx.state.real_time)`（§3.7 策略接口，**is_available 为带参方法而非 no-arg property——时间窗口比较需显式传当前 real_time，ISSUE-001**；可用性已含生命周期激活状态）
  ...
  - **`pool_quota.py:45-48` 的配额判定（REVIEW-R1-FIX: ISSUE-002）**：`pid = pool.id; drawn = ctx.pool_draw_counts.get(pid, 0)`——逐池统计键全限定化后 `ctx.pool_draw_counts` 键为 `{banner_id}.{pool_id}`，裸 pid 查询**恒 0、配额永不满足、持续抽到资源耗尽**。改为遍历 `ctx.banners` 读 `banner.pool_draws.get(pool_id)`（§3.7 已暴露，pool_id 为 Banner 内裸池 id、非全限定键）判配额。**`pool_quotas` 参数键语义钉死（REVIEW-R1-FIX: ISSUE-327）**：`pool_quotas`（PoolIntMapParam，pool_quota.py:14/18，用户配置键，读侧 `self.pool_quotas.get(pid)` :46/:55）在多 Banner 各含同名 'main' 时裸键歧义——裸 'main' 配额会同时作用于所有 Banner 的 main 池、配额判定串池。裁定：**`pool_quotas` 键改全限定 `{banner_id}.{pool_id}`**（与逐池统计键/`ctx.pool_draw_counts` 同键空间，Ph1a 迁移后用户配置写全限定键）；单 Banner 单池场景裸键兼容保留（`banner.pool_draws` 裸 pool_id 反查后仍命中，不强制改写）。Ph9 补「多 Banner 同名池」配额等价回归（配额不串池）
  ...
  - **4 策略 `_pool_needs_target` 匹配口径（REVIEW-R1-FIX: ISSUE-303/315/321）**：smart.py:61 / pity_reserve.py:43 / pool_quota.py:49 / stop_on_target.py:52 的 `_pool_needs_target(pool.id, ctx)` 以 `pool_id in t.pool_ids` 判断目标卡归属——**事实修正（ISSUE-321）**：原子提交态（构造桥 `Banner(id=p.id, pools={'main': p})`）下 `active_pool` 返回原 Pool 对象、`pool.id` 为**旧裸池 id**（如 'pool_c1'）而非字典键 'main'，与 `TargetCard.pool_ids`（card_defs 回填的裸 id）恰好命中——原子提交态不崩，掩盖了跨阶段冲突（ISSUE-315）；**Ph3 后 `card_defs[].pools` 变全限定键、Ph6 后 `pool.id` 变 pools 字典键**，裸 `pool.id in t.pool_ids` 届时静默失效。匹配参数改为 **`banner.id`**（原子提交态 `Banner(id=p.id, ...)` 使 banner.id == 旧裸池 id、Ph6 后 == Banner id，配合 `TargetCard.pool_ids` 钉死为 banner 级键——ISSUE-315 的 `k.split('.')[0]` 取段规则——恒可命中），`_ensure_pool_to_targets` 的缓存键随之以 banner.id 段对齐；Ph9 增加 4 策略在 banner 模式的目标追卡回归用例（见 §3.12 Ph9）
  - **`draw_target.py` 的 `pool_id` 参数匹配口径钉死（REVIEW-R1-FIX: ISSUE-326）**：draw_target 的 `pool_id` StrParam（draw_target.py:16/24/34 `pool.id == self.pool_id`）在 banner 模型下改匹配 **`banner.id`**（与 target_hunting 的 `target_pool_ids`、§3.4 ISSUE-303 的 `_pool_needs_target` 改 `banner.id` 同口径——原子提交态 banner.id == 旧裸池 id、Ph6 后 == Banner id，恒可命中）；`pool_id=''`（worst_impact.py:150 默认传参）保持「匹配任意」路径不变。**必须显式裁定，否则任何非空配置的 `pool_id`（如 'main'）在 banner 模型下静默匹配不到任何池**（pools 字典键 'main' 非 banner.id），draw_target 恒 WaitAction、永不抽卡——它是 worst_impact 的唯一消费方，最差影响分析随之退化。worst_impact.py:150 传参语义同步为「空串 = 任意 Banner」不变
  - fixed_count / target_hunting 目前只读 `ctx.current_pools`（无时间字段依赖）——**一并纳入 Ph1a 强制迁移（REVIEW-R1-FIX: ISSUE-003/321）**，原「可不迁移但须回归验证」取消。原因：`current_pools` 由 `active_banners[*].active_pool` 推导填充，元素 `.id` 为 Pool 对象自身 `.id`——**原子提交态为旧裸池 id（如 `'pool_c1'`）、Ph6 多池 banner 模式下才为 pools 字典键（如 `'main'`，非全局唯一标识）（事实修正，ISSUE-321）**——fixed_count.py:27 `DrawAction(pool_id=ctx.current_pools[0].id)` 在 Ph6 后得 `pool_id='main'`，多 Banner 共存时反查不唯一（§3.5 要点 5 的反查规避依赖 Ph1a 双字段，但这两个策略此前被豁免，自相矛盾）；target_hunting.py:25 `[p for p in ctx.current_pools if p.id in self.target_pool_ids]` 的 `target_pool_ids` 是用户配置的旧裸池 id（如 `'genshin_limited'`），Ph6 后与 `'main'` 永不匹配 → `target_pools` 恒空 → 策略静默只 WaitAction、永不抽卡。强制迁移理由以 **Ph6 后口径**为准，Ph1a 一并迁移避免二次迁移。迁移内容与 8 策略一致：改读 `ctx.banners` + `DrawAction(banner_id=..., pool_id=...)` 双字段；`target_pool_ids` 参数改匹配 `banner_id`（兼容旧裸池 id 时按 `banner_id` 段匹配）。`current_pools` 保留仅供遗留只读，无内置策略再依赖
...
```python
# config.toml 从 [[pools]] 一次性改写为 [[banner]]（Ph1 前置迁移阶段，见 §3.12）：
# - 每个旧 [[pools]] → 一个 [[banner]]，pool 字段原样进 [[banner.pool]]（id="main"）
...
# 迁移后解析器只认 [[banner]]——新建仅认 [[banner]] 的解析路径（现状代码库中不存在 `_wrap_pools_as_banners()` 函数、本无自动包装，无删除对象，REVIEW-R1-FIX: ISSUE-308）。
```
...
> 5. **策略→服务动作契约（REVIEW-R1-FIX: ISSUE-006/004）**：`core/action.py` 的 `DrawAction` 新增 `banner_id: Optional[str] = None` 字段，且 `pool_id` 改为 `Optional[str] = None`（**ISSUE-004——现状 action.py:15-16 的 `pool_id: str` 无默认必填，`DrawAction(banner_id=banner.id)` 会 TypeError；给默认 None 后省略合法**）。派发规则：`banner_id` 优先；`banner_id=None` 时按 `pool_id` 反查唯一 Banner（旧单池兼容路径）；`pool_id=None` 时服务层经 `banner.active_pool` 路由（§3.5 伪代码 `pool = banner.active_pool` 已是此行为，与 action.pool_id 无关）。多 Banner 各含同名 `main` 时裸 `pool_id` 全局重复、映射歧义，故 Ph1a 策略迁移强制全部改写为 `DrawAction(banner_id=..., pool_id=...)` 双字段（显式 pool_id 为推荐写法，但可省——§3.7 示例合法）。**显式 pool_id 的派发语义钉死（REVIEW-R1-FIX: ISSUE-316）**：单活跃池模型下显式 `pool_id` **不参与池选择**——派发恒取 `banner.active_pool`（伪代码 `pool = banner.active_pool` 从不读 action.pool_id），无法选择非活跃池（送抽插入期策略指定 free_10pull 的前提是 lifecycle 已 switch_to 使 free_10pull 成为活跃池，无需显式指定）；显式 `pool_id` 的**唯一功能角色**是 `banner_id=None` 时按全限定/裸 id 反查唯一 Banner（旧单池兼容路径）。Ph1a『双字段强制』的含义是**必须带 `banner_id`**（规避多 Banner 同名 main 时裸 pool_id 反查歧义），`pool_id` 字段本身对池选择无功能角色、可省；§3.7『显式双字段写法亦可』指反查歧义规避而非池选择。Ph9 增加显式 `pool_id` 派发用例（`banner_id=None` + 裸 `pool_id` 反查唯一 Banner 单池路径，ISSUE-316）主循环补全 **NonDrawAction 分支**（现状 gacha_service.py:367-368 → `_apply_non_draw` L87-121）：定轨切换/取消的 `action.params['pool_id']` 定位改为 `{banner_id}.{pool_id}` 全限定键（§3.11.3 命名空间），`pity_engine.get_behaviors_for_pool(pool_id)` 同步传全限定键。
> 6. **池结束快照机制迁移（REVIEW-R1-FIX: ISSUE-003）**：现状 `pool_end_times_sorted`（gacha_service.py:211-214，读 `Pool.available_until`）与 `collector.on_pool_end()`（L386-414）随字段退役失效。改为 `banner_end_times_sorted = sorted([(b.id, b.available_until) for b in banners.values() if b.available_until], key=lambda x: x[1])`，`on_pool_end(pid, ...)` → `on_banner_end(banner_id, ...)`，写 `CompactResult.banner_end_resources` / `banner_end_pity_states`。**单位（ISSUE-001）**：`banner.available_until` 为秒（§3.2「时间单位规范」），与 `real_time`（秒）同单位，`on_banner_end` 在 `real_time >= available_until` 触发——不再有现状秒、计划天 86400 倍错位（否则第 21 秒而非第 21 天触发池结束快照）。**P62 可达过滤（gdr.py:1115-1121）、per_pool_analysis.py:58/120/186 的 pool_end_times 累计快照、`core/vulnerability.py` 的 `pool_end_resources` / `pool_end_pity_states` 消费端一并迁移**（键语义：banner_id 取代 pool_id，Ph7 同步；vulnerability.py 若漏改，字段改名后 `r.get('pool_end_resources', {})` 静默拿到空 dict → `all_pool_ids` 为空 → `_fit_vulnerability_pava` 返回空 VulnerabilityAnalysisResult，脆弱性分析无声退化——REVIEW-R1-FIX: ISSUE-005）。**on_banner_end 兼作 Banner 关闭标记（REVIEW-R1-FIX: ISSUE-001）**：`on_banner_end` 触发（`real_time >= available_until`）时，除写快照外**同时调用 `banner._exhaust()`（或等价关闭标记）**——时间窗口过期即永久关闭（§六 风险表「时间窗口最先、直接 exhaust banner」的引擎落点）；此后策略层 `banner.is_exhausted=True`、`is_available(real_time)` 亦为 False。抽取循环内即使 on_banner_end 尚未触发（策略持续抽卡不等待），过期 Banner 也已由 active_banners 过滤的 `is_available(state.real_time)` 时间比较排除，两路径互补（§3.2 is_available 定义 / §3.5 伪代码）。窗口未开 Banner（`real_time < available_from`）**不**触发关闭标记——仅由 is_available 时间比较排除，等待期到 available_from 后重新可抽。
...
> 8. **`build_strategy_context` 签名（REVIEW-R1-FIX: ISSUE-005）**：下方伪代码以 `banners=` / `all_banners=` 关键字调用，但 `build_strategy_context()`（core/strategy_context_builder.py:24）当前签名无这两个参数——直接调用将 TypeError。Ph5 必须为 `core/strategy_context_builder.py` 新增 `banners` / `all_banners` 参数并透传进 `StrategyContext`；`current_pools` / `all_pools` 保留，由 `active_banners[*].active_pool` 推导填充——但内置策略（含 fixed_count / target_hunting）经 Ph1a 已全部迁移到 `banners`，不再依赖 `current_pools` 的池 id 匹配。**口径修正（REVIEW-R1-FIX: ISSUE-321）**：`current_pools` 元素 `.id` 是 `active_pool` 返回的 Pool 对象自身 `.id`——原子提交态构造桥（§3.5 要点 10 `Banner(id=p.id, pools={'main': p})`）下为**旧裸池 id**（如 `'pool_c1'`），Ph6 多池 banner 模式（from_config_store 以 `pool_entry.id` 构造 Pool）下才为 **pools 字典键**（`'main'`/`'step1'`）。fixed_count / target_hunting 强制迁移的理由以 **Ph6 后口径**为准（池 id 为 Banner 内字典键、多 Banner 各含同名 `'main'` 时无法作为全局唯一标识、裸 id 反查歧义），Ph1a 一并迁移避免 Ph6 二次迁移。与 P58 M4a 共用同一函数签名，各自负责 `banners` / `_milestone_engine` 参数，避免合并冲突。
...
<!-- REVIEW-FIX-PREV: AUDIT-BREAK-3 -->
> 10. **GachaService 构造桥（REVIEW-R1-FIX: GATE-3_依赖顺序，补充）**：`GachaService.__init__` 保持 `pools` 形参，但内部**双型收纳**为运行时 Banner 字典 `self._banners`：元素为 `Pool` → 就地单池包装 `Banner(id=p.id, name=p.name, pools={'main': p})`（键 `{pool_id}.main`，与 §3.4 [[pools]] 兼容包装一致；时间窗口 None 兜底，见 §3.13.4）；元素为 `Banner` → 直接收纳 `{b.id: b}`。任一分支保证原子提交后 `self._banners` 非空——`_run_single`（batch_simulator.py:228 传 `env.pools`）与现有 10 处测试（test_gacha_service.py:32/51/78/101/121、test_batch_service.py:25、test_batch_draw.py:171/212/260/314）直接构造 `GachaService([pool],...)` 均有池可路由。**桥与数据源分离**：Ph6 前 `List[Pool]` 路径的包装 Banner 时间窗口为 None（原子提交→Ph6 间时间窗口语义经 `PoolSchedule`/`end_time` 部分保留，banner 级窗口随 Ph6 从 `store.banner` 恢复）；Ph6 后 `from_config_store` 从 `store.banner` 构建运行时 Banner（多池/lifecycle/时间窗口），`SimulationEnv.pools` 承载 `List[Banner]`、`_run_single` 调用签名不变、`__init__` 直接收纳——任何阶段 `self._banners` 非空。
> **原子提交→Ph6 间逐池窗口过滤缺口（REVIEW-R1-FIX: ISSUE-102）**：本要点「部分保留」仅覆盖 `end_time`（停止条件）与 `banner_end_times`（快照）；**不覆盖逐池抽取可用性过滤**。构造桥包装 Banner 窗口为 None 时 `banner.is_available(state.real_time)` 恒 True（available_from/until 均 None，时间比较恒真）、`active_banners` 含全部池，策略可抽取时间窗口外已关闭的池；旧模型 `current_pools` 按 `available_from`/`available_until` 逐池过滤（gacha_service.py:224-226），样本 config 错峰池（pool_c1 0-21 天 / pool_c2 21-42 天）在中间态 day 0 全被抽取，抽数分布/目标卡达成时间线/GDR 改变。**此为原子提交→Ph6 间的已知中间态行为分歧**（Pool 字段已删、窗口仅存于 PoolSchedule 且构造桥不接；为「无历史包袱一次性迁移」精神不引入 Ph6 后需删除的临时窗口通道），处置：① **验收范围收缩**，GATE-3『首提交后现有 `[[pools]]` 行为完全不变』限定为「非时间窗口相关行为不变」（pytest 全量通过 + 无错峰窗口场景等价，见 §3.12 交付单元注）；② **Ph4b 的 golden 等价对照改排 Ph6 之后**，Ph6 前 from_config_store 无窗口恢复、对照必然失败（见 §3.12 Ph4b）；③ 逐池窗口过滤等价性由 Ph6 后恢复窗口的 Ph1-base golden 对照锁定（§七 验收）。**例外：`worst_impact` 不受此中间态影响（REVIEW-R1-FIX: ISSUE-302）**——其 `prepare_simulation_config` 随 Ph1a 改为 from_dict 直构带 `available_from`/`available_until` 的 Banner（不经构造桥 None 窗口路径），99 池错峰语义在原子提交态即完整保留，不属本缺口范围（§3.12 Ph1a / §四 波及范围）。
<!-- REVIEW-R1-FIX: ISSUE-102 -->
...
> 12. **原子提交态 pity 键一致性（REVIEW-R1-FIX: ISSUE-021/AUDIT-BREAK-3，阻塞 2）**：Ph2 将 gacha_service 的 PityEngine 查询键（`get_spec`/`before_draw`/`after_draw`）改为全限定 `{banner_id}.{pool_id}`，但 `_build_pity_engine_from_gui`（batch_simulator.py:139-158）构建的 `pool_specs` 键在 **Ph6 前仍是裸 `pool.id`**（from_config_store 构造的 Pool 带旧裸 id）。原子提交态（Ph1-Ph2 落地、Ph6 未落地）下 gacha_service 以 `{旧pid}.main` 查询 → `pool_specs.get` 为 None → `before_draw` 保底调整静默跳过 → **soft pity 静默失效**，与 GATE-3『首提交后 pytest 全量通过』及『现有 `[[pools]]` 行为完全不变』验收直接冲突。处置：**pool_specs 键的全限定化提前并入原子提交**（随 Ph2 或构造桥一起落地）——原子提交态 `_build_pity_engine_from_gui`/`from_config_store` 的 pool_specs 键改为 `{pool.id}.main`（单池包装口径），全限定键 fnmatch（§3.11.3，ISSUE-005；D4 一次性迁移后仅全限定键，无三路兼容）同步实现；Ph6 起键由 banner 池展开（多池/lifecycle）自然承接，无二次迁移。此改动把原 Ph6 的一部分（pool_specs 键全限定 + 全限定 fnmatch）前移进原子单元，Ph6 仅保留「数据源改 store.banner + 多池展开 + featured/ssr/rarity 来源」。**独立构建器补列（REVIEW-R1-FIX: ISSUE-301）**：`worst_impact._build_pity_engine`（worst_impact.py:443-510）的 pool_specs 键同为裸 `_worst_impact_pool_N`，须随本要点一并改为 `{pid}.main`（其经 from_dict 直构、不经 from_config_store，Ph6 迁移对它无效，故并入原子提交而非 Ph6，详见 §3.11.3）。
> **PityDef.pools 裸 id→全限定键改写随原子提交落地（REVIEW-R1-FIX: ISSUE-103）**：匹配侧改全限定键（`{pool.id}.main`）并入原子提交后，PityDef.pools 的裸 id（如 `["pool_c1"]`）仍待 Ph4b 改写全限定键（`["pool_c1.main"]`）——原子提交态 `fnmatch('pool_c1.main', 'pool_c1')` = False，非 `'*'` 的裸 id 绑定静默失配、soft pity 静默失效，与「旧 `[[pools]]` + 旧 `PityDef.pools` 绑定等价」验收自相矛盾。处置：**PityDef.pools 裸 id→全限定键的配置改写（config.toml `[pity_defs]` 段，独立于 `[[pools]]` 段，随原子提交一并落地、从 Ph4b 前移）**；默认 `'*'` 不受影响（`fnmatch('pool_c1.main', '*')` 恒 True）；Ph4b 的 PityDef.pools 改写项移除（见 §3.12 Ph4b）。原子提交态即「旧 `[[pools]]` 段 + 全限定 PityDef.pools」，与 `_build_pity_engine_from_gui` 全限定键匹配同口径。
<!-- REVIEW-R1-FIX: ISSUE-103 -->
...
def run_simulation(self, ...):
    ...
    for iteration in range(max_iterations):
        ...
        # is_available(real_time)（REVIEW-R1-FIX: ISSUE-001）：显式传入当前 state.real_time——
        # 时间窗口未开（available_from 未到）/已关（available_until 已过）的 Banner 在此被排除，
        # 与现状 gacha_service.py:224-226 逐池 real_time 过滤语义等价；策略不可再抽这两种状态的 Banner
...
- **P58 engine 跨进程传输（REVIEW-R1-FIX: ISSUE-329，契约空白补全）**：`register_milestone_engine(notifier, engine)` 的 `engine` 对象必须在 `_run_single`（worker 进程内）可获取。`_wk_init`（batch_simulator.py:175-205）仅 pickle `env` 注入全局 `_wk_env`，§5.4 的模块级全局 `_milestone_engine` 在 Windows spawn 下 worker 模块全局重置为 None——**`SimulationEnv` 新增 `milestone_engine=None` 字段（带默认值，与 `banner_defs` 同 pickle 兼容机制，ISSUE-011）**，`_run_single` 装配前从 `env.milestone_engine` 取出注入 `register_milestone_engine(notifier, engine)`；父进程 `run_batch_parallel` 侧在构造 env 时写入。P58 未实施期该字段为 None、`_on_after_draw` 内 `if _milestone_engine` 守卫跳过，不影响 P61 自身装配。
...
# ... rewards ...
```
（迁移说明：旧 `[[pools]]` 一次性改写为 `[[banner]]` + 嵌套 `[[banner.pool]]`，见 §3.4 / §3.12 迁移阶段；时间窗口字段见 §3.13.4。）
...
id = "full_example"
name = "完整示例"
max_draws = 200                    # Banner 级总抽数硬上限（可省，0=无限制——解析边界 _build_banners 归一化为
                                   #   None（0→None，None=无上限；0 直接落引擎则 _total_draws >= 0 恒 True、
                                   #   首抽即自动 exhaust，REVIEW-R1-FIX: ISSUE-331））——引擎自动执行，
...
```python
# ── 可用性 ──
banner.is_available(ctx.state.real_time)  # bool: 当前是否可以抽（REVIEW-R1-FIX: ISSUE-001——
                        #   is_available 为带参方法：时间窗口比较需显式传当前 real_time，
                        #   策略经 ctx.state.real_time 获取；窗口未开/已关 Banner 均返回 False）
...
# ── 待处理转换（策略决策核心）──
banner.pending_transitions(ctx.state.real_time)  # List[TransitionPreview]: 带参方法（REVIEW-R1-FIX: ISSUE-330——
                        #   time_window 条件的 remaining/current_value 需当前模拟时间，与 is_available(real_time) 对称，
                        #   策略经 ctx.state.real_time 传入）；按 remaining 升序排列（-1 不可预估项排最后，ISSUE-305）
```
...
for banner in ctx.banners:
    for pt in banner.pending_transitions(ctx.state.real_time):
        ...
        if pt.remaining >= 0 and pt.remaining <= 5 and pt.switches_current:
            # pt.remaining 单位（REVIEW-R1-FIX: ISSUE-317）：pool_draws/banner_draws 为抽数、
            # time_window 为【天】（秒差 /DAY 后向上取整，见 §3.2 TransitionPreview）——≤5 即
            # 5 抽或 5 天内垫刀；若 time_window 直接返回秒则判据差 86400 倍、垫刀决策失效
...
class BannerPoolEntry:
    ...
    max_draws: Optional[int] = None     # None=无上限；TOML/UI 层「0=无限制」在 _build_banners 解析边界归一化为 None
                                        # （REVIEW-R1-FIX: ISSUE-331）
...
**`store.banner` 是运行时唯一数据源（D3/D4 裁决，2026-08-02）。** 解析器只认 `[[banner]]`，`store.banner.banners` 为唯一存储；新建仅认 `[[banner]]` 的解析路径（现状解析器本就无 `_wrap_pools_as_banners` 自动包装函数，旧 `[[pools]]` 随 config.toml 一次性迁移移除，见 §3.4，REVIEW-R1-FIX: ISSUE-308）。`store.pools` **属性化**为只读展平视图（见下条），不再有「保留供旧代码读取兼容」的字段语义。

**`store.pools` 只读展平视图（D3 裁决）**：`ConfigStore.pools` 由字段改为 **`@property`**，遍历 `store.banner.banners[*].pools[*]` 展平为 `PoolEntry` 列表（`pool_id` 取 `{banner_id}.{pool_id}`；cost/batch_size/rewards 透传；**`name` 来源钉死（REVIEW-R1-FIX: ISSUE-324）**——展平 `PoolEntry.name` 取 `banner.name`（Banner 级 name 唯一；`BannerPoolEntry` 无 name 字段，不拼接 pool_id，否则图表标签含 'main' 后缀退化）。`analysis_panel._get_pool_names`/`_strip_pid`（analysis_panel.py:209/:1052）以展平视图 pool_id/name 构建图表标签——name 有值时图表显示中文池名、banner 模式下 label 按 `banner_id.pool_id` 拆分（`_strip_pid` 取 `.` 前段），Ph3 实施时验证 label 非原始全限定键；**「消费方零改动」的 D3 表述依赖 name 有值，此透传为前置条件**（REVIEW-R1-FIX: ISSUE-324，登记 §四 analysis_panel 行）；**enabled/distribution/featured_card_ids/epitomizable_cards/exchange_card_id 逐项透传——REVIEW-R1-FIX: ISSUE-314，漏透传则对应读侧消费静默退化：`config_panel.py:1268-1282` `_populate_selected_card_combo` 遍历 `self._store.pools` 读 `pool.epitomizable_cards` 填充 targeted 家族『初始定轨』下拉，漏透传则 banner 模式下该下拉恒空、定轨功能静默失效；`enabled` 漏透传则 gdr.py:510-514 的可达过滤 enabled 过滤失效、`distribution`/`featured_card_ids`/`exchange_card_id` 漏透传则相应消费端退化为空**）；Banner 级 `available_from`/`available_until` 回填为 start_day/end_day——**单位换算（REVIEW-R1-FIX: ISSUE-333）**：`available_from`/`available_until` 为秒（§3.2 时间单位规范），而 `PoolEntry.start_day`/`end_day` 为「天」（下游 `core/worst_impact.py:390` `pool_duration_days = pe.end_day - pe.start_day`、`core/retreat_config.py:23` `offset_day` 兜底 `start_day + 21`、`config_panel.py:3243` 预览 `p.end_day - p.start_day` 均以天数消费）——回填经 `// DAY` 换算（`start_day = available_from // DAY`（if not None）；`end_day = available_until // DAY`（if not None else None）），与保存边界 `save_toml` 的 `// DAY` 同口径；直接透传秒值则下游 86400 倍错位（`pool_duration_days=1814400` 而非 21））。**end_day=None 展平规则（REVIEW-R1-FIX: ISSUE-002）**：永久 Banner（`available_until=None`）展平回填 `end_day=None`（透传 None，不填 0——0 会被下游误判为「第 0 天结束」）。**两个展平视图消费端必须补 None 守卫**（否则 Ph3 后永久 Banner 成为常规形态即回归，旧格式 end_day 默认 21 不触发）：① `core/worst_impact.py:390` `_prepare_pool_info` 的 `pool_duration_days = pe.end_day - pe.start_day`——`pe.end_day` 为 None 时 `None - int` 抛 TypeError（`_prepare_pool_info` 以 `[pe for pe in self.store.pools if pe.enabled]` 读展平视图、取最后一个启用池为参考池，配置含永久 Banner 时最差影响分析必崩）——改为 `pe.end_day is None` 时按现状 None 兜底语义取 `pool_duration_days = 21`（等价 `(start_day + 21) * DAY`，与 §3.13.4 eff_end 兜底同口径）；② `core/retreat_config.py:23` `RetreatConfigBuilder.build` 的 `offset_day = from_pool.end_day if from_pool.end_day > 0 else (from_pool.start_day + 21)`——退避点选永久池时 `None > 0` 抛 TypeError——改为 `offset_day = (from_pool.end_day if from_pool.end_day is not None and from_pool.end_day > 0 else (from_pool.start_day + 21))`；另 `:47` 重建写侧 `end_day=p.end_day - offset_day` 对永久池 `None - int` 同抛 TypeError——改为永久池（end_day None）在截断视图中保持 `end_day=None`（Ph3 重建写侧迁移到 `truncated.banner.banners` 时对 `available_until=None` 透传 None，见 §3.12 Ph3）。**读侧消费方零改动**（属性名/结构不变，仍是 `List[PoolEntry]`）：analysis_panel.py:177/1593/1619、gacha_panel.py:283、main_window.py:276/387/431、resource_search_panel.py:276-278、worst_impact_panel.py:185、retreat_panel.py:217/271、gdr.py:512 照常读 `store.pools`，拿到的是从 banner 展平的结果，不拿空列表。**cli.py:149 例外（REVIEW-R1-FIX: ISSUE-003——pool_id 值级变化而非仅列表非空性）**：cli.py:148-150 的兜底目标 id `actual_target_ids = [f"{store.pools[0].pool_id}_ssr"]` 从 pool_id **值**推导，Ph3 展平后 pool_id 为全限定 `{banner_id}.{pool_id}`、兜底生成 `{banner}.main_ssr` 与任何真实卡 id 不匹配 → ssr 计数恒 0、CLI 无 `[[targets]]` 时 GDR 静默 0%。改取 banner 段 `store.pools[0].pool_id.split('.')[0]`（再拼 `_ssr`）或改从 `store.banner` 推导兜底目标（见 §3.12 Ph3）。

<!-- REVIEW-R1-FIX: AUDIT-BREAK-1 -->
**写侧统一走 `store.banner`（D3 裁决，替代 dual-write）**：`store.pools` 属性化只读后不可写。原写侧全部改走 `store.banner`：`config_toml.py:1107`（`[[pools]]` 解析 append）随一次性迁移删除；`config_panel.py:3879`（整体重绑）、`:3385/:3925`（append）改为写 `store.banner`——**随 {Ph3, Ph4} 交付单元前移落地（AUDIT-BREAK-1，与 ISSUE-311 同源协调）**：Ph3 将 `store.pools` 属性化只读后，config_panel 写侧（set_config :3385 append / apply_to_store :3879 `store.pools = []` 重绑、:3925 append）与 config_toml `_build_pools`（config_toml.py:1107）同时进入中间态——`:3879` 对无 setter 只读 property 赋值抛 **AttributeError**、`:3385/:3925` 对每次访问重建的派生列表 append **静默丢池**（保存后 store.pools 空、GDR/GUI/模拟拿到空数据且无异常）。原 ISSUE-311 仅协调 config_toml 写侧，**config_panel 写侧此前无任何处置，Ph3 落地至 Ph8c 迁移之间的 GUI 保存路径直接崩溃，与「任何阶段系统可运行」承诺冲突**。处置（二选一，与 config_toml 写侧同 commit 约束）：① config_panel 写侧随 {Ph3, Ph4} 同 commit 迁移到 `store.banner`（BannerEntry/BannerPoolEntry，从 Ph8c 前移——Ph8c 仅保留 Tab UI 重写与读侧 Banner 适配，见 §3.12 Ph3/Ph8c）；② 按 ISSUE-311 第二选项在 Ph3→Ph4 窗口临时保留 `store._pools` 内部可变字段供未迁移写侧写入、Ph4 一次性切纯展平视图。不得让「Ph3 独立提交、config_panel 写侧 Ph8c 后补」的中间态存在。**`core/retreat_config.py:39/42`（`RetreatConfigBuilder.build` 的 `truncated.pools = []` / `truncated.pools.append(PoolEntry(...))` 池重建写侧，被 retreat_search.py:134 调用）随 Ph3 改为写 `truncated.banner.banners`（BannerEntry/BannerPoolEntry）**（REVIEW-R1-FIX: ISSUE-101：Ph3 属性化后 `:39` 对无 setter 的 property 赋值抛 AttributeError、`:42` 对每次访问重建的派生列表 append 静默丢池）。**无 dual-write、无 Legacy mirror、无冗余镜像**——单一数据源，读侧经属性视图兜底。原 ISSUE-010「`store.pools` 保持可变字段」约束随 D3 裁决作废。
<!-- REVIEW-R1-FIX: ISSUE-101 -->

**P62 可达过滤数据源（REVIEW-R1-FIX: ISSUE-009）**：`filter_target_specs_by_obtainable`（gdr.py:510-514）当前遍历 `store.pools` 构建 `pool_start[p.pool_id] = p.start_day`——banner 模式（`store.pools` 为空）会把所有目标卡判为不可达，4 个 `_obtainable` GDR 分母退化。改为遍历 `store.banner.banners`（`enabled` 判定 → `banner.available_from`）构建可达映射，与 §3.13.2 复刻时间线排序、§3.13.4 时间窗口上移共用同一数据源（Ph3 实施，文档同步表已登记说明；**比较单位由天修正为秒的行为影响——晚开池可达性收严，见 §3.13.4 ISSUE-307 声明**）。**`pool_start` 键格式为全限定 `{banner_id}.{pool_id}`（对每个 banner 的每个 pool 展开，`key = f"{banner.id}.{pool.id}"`）——与 `_backfill_card_pools`（config_toml.py:1178-1184，load_toml:78 内置步骤）在 Ph3 展平视图下写入 `card_defs[].pools` 的全限定键同口径**（该函数遍历 `store.pools`、写入 `pool.pool_id`，Ph3 属性化后 pool_id 即全限定键），gdr.py:508 的 `card_pools` 与 `pool_start` 同键空间、`pool_start.get(pid)` 不落空（REVIEW-R1-FIX: ISSUE-310——键空间错配会让 4 个 `_obtainable` GDR 分母静默退化）。

<!-- REVIEW-FIX-PREV: AUDIT-BREAK-2 -->
<!-- REVIEW-FIX-PREV: AUDIT-BREAK-4 -->
**保存侧统一写 `[[banner]]`（REVIEW-R1-FIX: ISSUE-012，D3/D4 后简化）**：`save_toml()`（config_toml.py:89、_save_templates_and_pools :248-277）从 `store.banner.banners` 写 `[[banner]]`（含 `[[banner.pool]]` / `[[banner.lifecycle]]`），**不再写 `[[pools]]`、无双路径分支**。配置迁移（`[[pools]]` → `[[banner]]`）在 Ph1 前置一次性完成（见 §3.12 迁移阶段）；保存前无需 `flattened_pools` 快照（store.pools 只读视图不参与写，无文件丢失风险）。
- `pool_type` / `rerun_of` 解析残留排期 Ph4 清理：`PoolEntry.pool_type` / `rerun_of` 字段删除；`_build_pools`（config_toml.py:1032/1070-1071/1094/1102）、复刻复制（:1110-1115）、写出（:260-277）同步移除，旧 TOML 中这两个键解析时忽略（不报错）。**字段删除影响清单（读侧+写侧，ISSUE-007）**：`core/retreat_config.py:42-62`（`PoolEntry` 重建同时读 `p.pool_type`/`p.rerun_of` 并以之为关键字构造——Ph1a/Ph4 同步改不传）、`gui/config_panel.py:3241/4059`（读/写 pool_type）、`config_toml.py:1094`（写 pool_type）、**`gui/main_window.py:388`（`pool_type = pe.pool_type or (pe.bindings.get('type', '角色') ...)`——`pe.pool_type` 访问在 Ph4 删字段后 AttributeError，**改写口径修正（REVIEW-R1-FIX: ISSUE-334）**：展平视图**不透传 bindings**（§3.9 透传清单不含 bindings、`BannerPoolEntry` 无该字段——banner 模式下 `pe.bindings` 恒空 dict），原「改为 `pe.bindings.get('type', '角色') if pe.bindings else '角色'`」在 banner 模式恒返回 '角色'、武器/兑换池展示静默退化——**明确设计取舍**：banner 模式类型不来自 bindings，`main_window.py:386-389` 的 pool_types dict 本就是 Ph7 `result.pool_types` 的展示消费端（§3.13.1 键/值迁移）、随 `result.pool_types` 由 output/random 推导（`derive_type`）供给即可；`config_panel.py:3241` 的 get_config 预览摘要与 `:4059`（`_refresh_from_store_impl` 构建 pools_data）同理改从 output/random 推导类型或**显式接受恒 '角色' 的展示退化**（纯展示、不崩溃）——两者择一明确，不得留下对空 bindings 的隐性依赖，AUDIT-BREAK-4；config_panel 写侧构造 `PoolEntry(... pool_type=...)` 的两个点（set_config :3389 / apply_to_store :3929）随 {Ph3, Ph4} 的 store.pools 写侧迁移到 store.banner 一并消失（AUDIT-BREAK-1）、`tests/core/test_batch_draw.py:24`（`PoolEntry(... pool_type='角色')` 关键字，Ph4 删字段后 TypeError，AUDIT-BREAK-4）、`tests/service/test_gacha_service.py:142`（`PoolEntry(... pool_type='角色')` 关键字，同 test_batch_draw.py:24，AUDIT-BREAK-2/4）**——随 Ph4 一并清理。
...
| `max_draws` | `QSpinBox` | Banner 级硬上限，0=无限制——**保存边界归一化为 None（0→None，None=无上限；0 直接落引擎则 `_total_draws >= 0` 恒 True、首抽即自动 exhaust，REVIEW-R1-FIX: ISSUE-331）**——引擎自动执行（`_total_draws >= max_draws` → 自动 exhaust，无需手写 lifecycle，ISSUE-303） |
...
**消费点连带清理（REVIEW-R1-FIX: ISSUE-005）**：`gui/main_window.py:276-277` 的 `inline_count = sum(1 for p in self._store.pools if not p.distribution_template)` 统计「使用内联分布（模板不匹配）」的池数并在保存/导出时显示状态栏提示——移除模板后 `PoolEntry.distribution_template` 恒为默认 `''`（Ph3 展平视图无从提供该字段，`BannerPoolEntry` 无 distribution_template）、`not p.distribution_template` 恒 True、提示恒显示（误导用户）。**随本小节一并删除或改写该提示逻辑**——展平视图无法提供 distribution_template、该统计失去语义；登记进 §3.12 Ph8c 与 §四 波及范围。
...
- **空勾选保存值（REVIEW-R1-FIX: ISSUE-328——与旧空文本→`('*',)` 语义相反，必须裁定）**：「全不选」（0 行勾选）保存为 `pools = []`（空列表 = **不绑定任何池**），**不得**沿用旧「空文本 → `('*',)`」逻辑（那会把「不绑定任何池」序列化为「绑定全部池」，语义反转）。引擎匹配语义钉死：空 `PityDef.pools` 元组时 `any(fnmatch(...) for ptn in pools)` 恒 False → 保底规则不匹配任何池（与「全不选」语义一致，规则不生效但不误伤全池）；「全选」仍生成 `*`（匹配全部）。两语义各自确定、不冲突。Ph8b 实现（保存分支显式区分「空勾选 → []」与「全选 → `*`」）与 Ph9 用例锁定（空勾选保存→加载不再勾选任何行、保底规则不触发；全选保存→加载全勾选）
- 「全选」勾选所有行（生成 `*` = 匹配全部）；「全不选」取消所有勾选
- `excludes_all_pity=True` 的 Pool 仍然可以勾选（绑定是声明性的），但引擎不应用保底

#### 3.11.3 引擎匹配逻辑与池标识命名空间

TOML `pools` 字段仍存储 fnmatch pattern。引擎匹配流程不变——遍历所有 Banner 的所有 Pool，用 `{banner_id}.{pool_id}` 与 pattern 做 `fnmatch`。**匹配规则（D4 一次性迁移后简化，REVIEW-R1-FIX: ISSUE-004/005）**：仅对全限定键 `{banner_id}.{pool_id}` 做全限定 fnmatch（无三路拆分）。旧裸池 id 模式（`pools=["genshin_limited"]`）随原子提交改写为全限定键（`["genshin_limited.main"]`，REVIEW-R1-FIX: ISSUE-103——与匹配侧改全限定键同口径落地、从 Ph4b 前移，见 §3.5 要点 12 / §3.12 Ph4b），因此不需要 banner_id 段 / 裸 pool_id 段兼容匹配。**实现面（ISSUE-005）**：`batch_simulator._build_pity_engine_from_gui`（batch_simulator.py:139-158）的 pool_specs 构建 fnmatch（:139-144 `any(fnmatch.fnmatch(pool.id, ptn) for ptn in pools_ptn)`）按全限定键匹配，PityDef.pools 在全限定键空间内 fnmatch（`fnmatch('genshin_limited.main', 'genshin_limited.main')` 为 True）；Ph9 用迁移后用例锁定保底绑定等价。
...
- **D4 一次性迁移后无包装退化情形**：全限定键统一为 `{banner_id}.{pool_id}`，无 `{pool_id}.main` 自动退化。旧裸 id 的 PityDef.pools（`["genshin_limited"]` / `["main"]`）随原子提交改写为全限定键（`["genshin_limited.main"]`）（REVIEW-R1-FIX: ISSUE-103，从 Ph4b 前移；Ph8b 实现 + Ph9 用例仍锁定迁移后绑定行为，REVIEW-R1-FIX: ISSUE-004）
...
- **worst_impact 独立构建器补列（REVIEW-R1-FIX: ISSUE-301）**：`worst_impact._build_pity_engine`（worst_impact.py:443-510，独立于 batch_simulator `_build_pity_engine_from_gui`）同样以裸 `_worst_impact_pool_N` 为 pool_specs 键（:487 `pid = f'_worst_impact_pool_{pool_idx}'`）——Ph2 后 gacha_service 经构造桥/from_dict 的查询键为 `{banner.id}.{pools 字典键}` = `_worst_impact_pool_0.main`，裸键查询 `pool_specs.get(pool_id)` 恒 None、`before_draw` 返回未调整概率、**保底静默失效**。**该构建器 pool_specs 键一并改为 `{pid}.main`（单池包装口径，与 `_build_pity_engine_from_gui`/`from_config_store` 原子提交态同口径），其 pdef.pools 的 fnmatch（worst_impact.py:491）同按全限定键匹配**——且最差影响分析经 `SimulationEnvBuilder.from_dict`（worst_impact.py:137）直构 env、不经 from_config_store，Ph6 迁移对它无效，故本项随原子提交（Ph1a/Ph2 时段）落地而非 Ph6；Ph9 增加最差影响分析保底触发回归用例（见 §3.12 Ph9）
...
- **命名空间超出 PityEngine（REVIEW-R1-FIX: ISSUE-002）**：同一 `{banner_id}.{pool_id}` 键空间同时用于 stats/collector/result_types 的逐池统计键（`pool_draw_counts` / `pool_card_counts` / `pool_pity_counts` / `draw_pool_ids` / `pool_types`），保证兼容模式（每个 `[[pools]]` 包装为含 'main' 内部池的 Banner）下多个 'main' 键在统计字典中不串池——否则跨 Banner 统计串池、保底重置归因（pool_card_counts/pool_pity_counts）与 GDR 分母错乱。详见 §3.8

### 3.12 实施阶段

| 阶段 | 内容 | 文件 | 预估 |
|------|------|------|:---:|
| Ph0 | `core/notifier.py` —— subscribe / emit / priority + **装配契约（REVIEW-R1-FIX: ISSUE-301）**：Notifier 由装配层（batch_simulator `_run_single` 构造 GachaService 处）创建并经新增构造参数注入 `GachaService.__init__`（`notifier=None` 时服务内 fallback 自建，批处理入口必须传共享实例）；装配层在构造服务**前**调用 **P58 装配函数** `register_milestone_engine(notifier, engine)` 注册 priority=0 订阅；**P58 engine 跨进程来源契约补全（REVIEW-R1-FIX: ISSUE-329）——`SimulationEnv` 新增 `milestone_engine=None` 字段（带默认值、与 `banner_defs` 同 pickle 兼容机制，worker 内 `_wk_env` 可读），`_run_single` 装配前从 `env.milestone_engine` 取出注入 `register_milestone_engine`；不得依赖 worker 模块级全局 `_milestone_engine`（Windows spawn 下重置为 None）**。**P61 + P58 共享基础设施**——Ph0 交付后两个计划可完全并行 | 新建 | ~30行+装配点 |
| Ph1-base | **基线固化（D3/D4 新增，等价性安全网）** —— 在 Ph1 动任何字段前，用**当前旧代码 + 现有 `[[pools]]` 配置**跑固定种子模拟（seed=42，n≥1000），把 CompactResult 序列化固化到 `tests/fixtures/baseline_pool_golden.json`。后续任何删字段/改键/迁移阶段，同种子重跑并逐字段对比该 golden，保证行为等价（CLAUDE.md「无历史包袱原则」要求的基线固化 + 等价对照） | 新增 fixture | ~30行脚本 + fixture |
...
<!-- REVIEW-R1-FIX: AUDIT-BREAK-2 -->
| Ph1a <!-- REVIEW-FIX-PREV: AUDIT-BREAK-1 --> | **策略迁移 + Pool 构造点迁移（新增，REVIEW-R1-FIX: ISSUE-001/008）** —— 8 个内置策略从 `pool.is_available_at`/`pool.available_until` 迁移到 `ctx.banners`（§3.4 迁移方案）并改 `DrawAction` 双字段（§3.5 要点 5）；**含 fixed_count / target_hunting（原「可不迁移」取消——Ph6 多池 banner 模式下 `current_pools` 元素 id 为 pools 字典键、非全局唯一（原子提交态为旧裸池 id，理由以 Ph6 后口径为准，REVIEW-R1-FIX: ISSUE-321），`target_pool_ids` 匹配改 `banner_id`，REVIEW-R1-FIX: ISSUE-003）**；**另含 pool_quota / pity_reserve 两个裸 pool.id 消费点（ISSUE-002）**——pool_quota 的 `ctx.pool_draw_counts.get(pool.id)`（全限定化后恒 0、配额永不满足）改读 `banner.pool_draws`（§3.7）；**另含 draw_target 的 `pool_id` StrParam 匹配口径改 `banner.id`（REVIEW-R1-FIX: ISSUE-326，与 worst_impact.py:150 默认传参 `pool_id=''` 匹配任意路径同步）与 pool_quotas 参数键改全限定 `{banner_id}.{pool_id}`（多 Banner 同名 main 配额不串池，REVIEW-R1-FIX: ISSUE-327）**；pity_reserve 的 `ctx.get_pity_probabilities(pool.id)`（查不到全限定 spec、保底阈值退化基础概率）改传全限定键或经 StrategyContext 新增 banner 维度接口（Ph5）；**另含 4 策略 `_pool_needs_target` 匹配口径（REVIEW-R1-FIX: ISSUE-303/315/321）**——smart.py:61 / pity_reserve.py:43 / pool_quota.py:49 / stop_on_target.py:52 以 `_pool_needs_target(pool.id, ctx)` 判断目标归属（`pool_id in t.pool_ids`）：**事实修正（ISSUE-321）**：原子提交态 `pool.id` 为旧裸池 id（构造桥下 active_pool 返回原 Pool 对象）、`TargetCard.pool_ids` 为 card_defs 回填的裸 id，裸匹配**恰好命中**（原子提交态不崩，掩盖跨阶段冲突）；Ph3 后 card_defs pools 变全限定键、Ph6 后 `pool.id` 变 pools 字典键，裸匹配届时静默失效——改为传 **`banner.id`**（原子提交态 banner.id == 旧裸池 id、Ph6 后 == Banner id，配合 TargetCard.pool_ids 钉死为 banner 级键——ISSUE-315 的 `k.split('.')[0]` 取段规则——恒可命中）；`service/config_service.py:52-60` 与 `core/worst_impact.py:195-207` 的 `Pool(...)` 关键字构造改删字段后签名（available_from/until 不再传、is_exchange 改 property）；`generator/schedule_generator.py:32-36` 经核实仅构造 `PoolSchedule`（core/schedule.py，字段保留、不受 Ph1 删 Pool 字段影响），**无需迁移**，从构造点清单移除（REVIEW-R1-FIX: ISSUE-306——原清单以「schedule_generator.py:34-35 存在 Pool 构造」为前提，属范围事实失准，§四 波及范围已同步修正）；**worst_impact 的 99 池错峰时间窗口改直接构造带 `available_from`/`available_until` 的 Banner（REVIEW-R1-FIX: ISSUE-302）**：worst_impact 走 `SimulationEnvBuilder.from_dict`（worst_impact.py:137）直构 `List[Pool]`、不经 from_config_store，Ph6 迁移对它无效——若 Ph1a 仅删 `Pool(available_from=...)` 关键字、窗口无替代承载，99 池错峰窗口永久丢失（构造桥包装 Banner 窗口为 None → 99 池恒可用、draw_target 只抽 pool 0、ConsecutivePoolTargetCondition 在 pool 0 窗口结束时因 pool 1 目标未抽即停机、`expected_pools` 恒≈1）。改为 from_dict 直构 `List[Banner]`：`Banner(id=pid, name=..., pools={'main': Pool(...)}, available_from=start_time, available_until=end_time)`（Banner 级窗口，构造桥对 Banner 元素直接收纳 `{b.id: b}`、窗口保留，原子提交态即生效）；`schedule_mgr`/PoolSchedule（worst_impact.py:204-208）与 `pool_targets` 保留供 ConsecutivePoolTargetCondition 停止条件使用；Ph9 增加最差影响分析 `analyze()` 级回归用例锁定 `expected_pools` 不退化（≈MAX_POOLS）；**直构 Pool 的 rewards 概率归一化（REVIEW-R1-FIX: ISSUE-319）**：worst_impact `_prepare_pool_info`（worst_impact.py:358-367）与 `_apply_custom_pool_config`（:410+）以 `prob = de.probability` / `d.get('probability')`（百分制，如 100.0/60.0）构造 Pool.rewards——is_exchange 改推导 property 后（§3.13.1 tuple 公式 `len(rewards) > 1 or rewards[0][1] < 1.0` **假定概率已归一化 0-1**，与 from_config_store `de.probability / 100.0`（batch_simulator.py:528）一致），单卡 100% 池 `100.0 < 1.0`=False 且 len==1 → random=False → is_exchange=True → `__post_init__` 不建 `_reward_list` → draw() 走 exchange 分支恒返回首卡、99 池连续目标判定被绕过（旧模型 is_exchange 字段=False、正常抽卡）。**改为直构 Pool 时 rewards 概率归一化 `prob = de.probability / 100.0`（两处构造点同改）**；**`service/config_service.py:25-58` 的 `export_pool_to_config` / `import_pool_from_config` 两个方法为死代码（REVIEW-R1-FIX: ISSUE-020/AUDIT-BREAK-1——全库 grep 无任何调用方，仅定义处命中）：`export_pool_to_config` 读 `p.available_from`/`p.available_until`（:36-37）、`import_pool_from_config` 以将删字段 `available_from`/`available_until`/`is_exchange` 为关键字构造 `Pool(...)`（:52-60），Ph1 删字段后两方法均必然 AttributeError/TypeError。处置：Ph1a **直接删除这两个方法**（死代码，无消费方，删后无影响面）；若保守保留类壳则删除方法体中的字段读写并标注废弃。AUDIT-BREAK-1 另注：Ph1 删字段后原子提交测试断裂属 GATE-3 已覆盖范围，本条仅补死代码清单**；**`service/batch_simulator.py` 的 `from_config_store`（:547-559）同属本清单但原计划漏列（REVIEW-R1-FIX: GATE-3_依赖顺序）——该处同样以将删字段为关键字构造 `Pool(...)`（`available_from=start_day*DAY` / `available_until=end_day*DAY` / `pool_type` / `is_exchange`），Ph1 删字段后首提交即 TypeError；Ph1a 一并删这四个关键字，时间窗口改由 PoolSchedule（:561-565）承载、`end_time` 计算（:568）不变，banner 模式数据源改造仍归 Ph6**；`core/retreat_config.py` 的 `PoolEntry` 重建**写侧**改不传 `pool_type`/`rerun_of`（retreat_config.py:50/54 现以已删字段为关键字构造——既 AttributeError 又 TypeError，ISSUE-007），读侧停止读 `PoolEntry.pool_type`——**与 Ph1 同 commit 交付（REVIEW-R1-FIX: GATE-3，见 §3.12 交付单元注）**；**策略层直接构造测试改写（AUDIT-BREAK-2，随原子单元同 commit）**——test_worst_impact_correctness.py:66/94/123（draw_target 3 测试，构造 :73/102/130）、test_strategy.py:11/_make_ctx 与 test_target_hunting_strategy、test_batch_draw.py:122/:130（SmartStrategy）、test_p69_strategy.py:223（`build_strategy_context(**base)`）四处改写面不在 AUDIT-BREAK-7 清单，Ph1a 迁移后必红——改经 Banner 包装构造 ctx.banners（详情见 §3.12 交付单元注 ⑥-⑨）；`StrategyContext.banners`/`all_banners` 与 `build_strategy_context` 新参数带默认值（Ph5，AUDIT-BREAK-2）| 修改 8 策略 + 6 文件 + 4 测试文件 | ~110行 |
| Ph1b <!-- REVIEW-FIX-PREV: AUDIT-BREAK-2 --> | `core/state.py` —— 删除 `GachaState.get_available_pools()`（REVIEW-R1-FIX: ISSUE-002；REVIEW-R1-FIX: GATE-3——方法体 `pool.is_available_at()` 随 Ph1 删字段即失效，必须与 Ph1 同 commit 交付），清理 `Pool` import 死依赖。**+ 同步删除 `tests/core/test_state.py::test_get_available_pools`（:73-82——构造 Pool 用将删 `available_from`/`available_until` 关键字 + 调已删方法，双断裂；REVIEW-R1-FIX: GATE-6_测试策略——删用例随本阶段并入原子单元，否则首提交后至 Ph9 前 pytest 必红，与 GATE-3『首提交后 pytest 全量通过』自相矛盾）+ 改写 `tests/service/test_gacha_service.py::test_env_builder_from_config_store_smoke`（:164 的 `env.pools[0].pool_type` 断言随 Ph1 删字段失效 → 改断言推导属性 `pool.output`/`pool.random` 或 §3.13.1 `derive_type`，REVIEW-R1-FIX: GATE-3_依赖顺序；**另 :142 的 `PoolEntry(... pool_type='角色')` 关键字——`PoolEntry.pool_type` 字段 Ph4 才删除，但同测试已在 Ph1b 改写范围，一并移除该关键字（At 阶段字段仍存在、移除无害），避免 Ph4 再断一次，AUDIT-BREAK-2/4**）** | 修改 | -13行 + 清理 |
...
| Ph2 <!-- REVIEW-FIX-PREV: AUDIT-BREAK-3 --> | `service/gacha_service.py` —— `current_pools` → `current_banners`（`Dict[str, Banner]`），banner 路由 + **保留逐抽结算管线**（spend/保底/统计/add_card/溢出/collector.on_draw，与现状 L259-365 一致）+ `after_draw` 单抽粒度 emit + **Notifier 装配**（`self._notifier` 创建 + P61 转换订阅 priority=1，ISSUE-001）+ **逐池统计键改全限定 `{banner_id}.{pool_id}`**（stats/collector/pool_types，ISSUE-002）+ **pool_specs 键全限定 + 全限定键 fnmatch（AUDIT-BREAK-3 阻塞 2，原子提交内必须与查询键同口径——`_build_pity_engine_from_gui`/`from_config_store` 的 pool_specs 键改 `{pool.id}.main` 并按全限定键 fnmatch，D4 一次性迁移后仅全限定键匹配、无三路兼容，否则原子提交态 `get_spec('{旧pid}.main')` 查裸键引擎为 None、soft pity 静默失效；原 Ph6 的 ISSUE-005 部分前移到原子单元，见 §3.5 要点 12 / §3.11.3）**+ **PityDef.pools 裸 id→全限定键改写随原子提交一并落地**（config.toml `[pity_defs]` 段 PityDef.pools 改全限定键，从 Ph4b 前移：匹配侧改全限定后旧裸 id 非 `'*'` 绑定 fnmatch 失配、soft pity 静默失效，REVIEW-R1-FIX: ISSUE-103）+ **collector.on_draw 传 `triggered_pity_name`**（取 DrawOutcome，ISSUE-003）+ **池结束快照机制迁移**（`pool_end_times_sorted`/`on_pool_end` → `banner_end_times_sorted`/`on_banner_end`，ISSUE-003）+ `pool_types` 数据源改造（ISSUE-004）+ NonDrawAction 全限定键分支（ISSUE-006）+ **WaitAction 分支等待期 `time_window` 评估**（real_time 推进后对 active_banners 调 `_check_transitions(real_time=...)`，REVIEW-R1-FIX: ISSUE-003）+ **`GachaService.__init__` 构造桥（REVIEW-R1-FIX: GATE-3_依赖顺序，契约见 §3.5 要点 10）——`pools` 双型收纳为运行时 Banner 字典 `self._banners`：元素为 `Pool` → 就地单池包装 `Banner(id=p.id, name=p.name, pools={'main': p})`（时间窗口 None 兜底）；元素为 `Banner` → 直接收纳 `{b.id: b}`——保证 `_run_single`（batch_simulator.py:228 传 `env.pools`）与现有测试直接构造 `GachaService([pool],...)` 后 `self._banners` 非空、`run_simulation` 可路由**——**与 Ph1/Ph1a/Ph1b/Ph1c/Ph5 同 commit 交付（REVIEW-R1-FIX: GATE-3，见 §3.12 交付单元注）**：本阶段无法独立落地——banner 路由需 Banner 类（Ph1c）、`build_strategy_context(banners=)` 需 Ph5、旧 `pool_end_times_sorted`/`current_pools` 字段读取在 Ph1 删字段后即崩、`self._banners` 需构造桥填补 | 修改 | ~120行变更 |
<!-- REVIEW-R1-FIX: AUDIT-BREAK-1/3 -->
| Ph3 | `core/config_store.py` —— `BannerEntry` / `BannerPoolEntry` / `LifecycleRuleEntry` / `BannerConfig` dataclass + `ConfigStore` 新增 `banner` 字段 + **`store.pools` 属性化为只读展平视图**（D3 裁决，替代 dual-write/flattened_pools 兼容视图）+ **`core/retreat_config.py` 的 `truncated.pools` 池重建写侧迁移到 `truncated.banner.banners`**（REVIEW-R1-FIX: ISSUE-101：`RetreatConfigBuilder.build` 的 :39 `truncated.pools = []` / :42 `append(PoolEntry(...))` 是 store.pools 写侧，属性化后对无 setter property 赋值 AttributeError / 对派生列表 append 静默丢池，同阶段改为 `BannerEntry(pools=[BannerPoolEntry(...)])` 写入 `truncated.banner.banners`，Banner 级 `available_from`/`available_until` 按原 start_day/end_day 偏移后写入以便展平视图正确回填；`from_pool_id` 定位（:15-17 遍历 `original_store.pools` 比对 `p.pool_id`）在展平视图下 pool_id 为全限定键 `{banner_id}.{pool_id}`，改为按 banner 展开匹配裸池 id；与 Ph1a 已排的同文件 PoolEntry 关键字迁移（:50/:54 不传 pool_type/rerun_of）叠加实施）**+ `_backfill_card_pools` 键空间适配登记（REVIEW-R1-FIX: ISSUE-310/315）**：Ph3 属性化后 `store.pools` 为展平视图，`_backfill_card_pools`（config_toml.py:1178-1184，load_toml:78 内置步骤）与 batch_simulator.py:656-662 写入 `card_defs[].pools` 的 pool_id 即全限定键 `{banner_id}.{pool_id}`——gdr.py:508 `card_pools` 与 §3.13.4 `pool_start` 同键空间（正确，无需改写入规则，但须在实施时显式登记该键空间变化）；**而 `TargetCard.pool_ids`（batch_simulator.py:188 `_wk_init` / :300 run_batch_parallel 由 `card_defs[idx]['pools']` 原样拷入）因此变全限定键、与 4 策略 `banner.id` 匹配（`'pool_c1' in ['pool_c1.main']`）恒 False、静默跳过目标追卡（ISSUE-315）——TargetCard 构造处取 banner_id 段（`k.split('.')[0]`，无段则原样）的规则随 Ph6 落地（见 Ph6 行），Ph9 增加 banner 模式目标追卡回归锁定**+ **`_populate_selected_card_combo` 数据源验证（REVIEW-R1-FIX: ISSUE-314）**：§3.9 展平视图补透传 `epitomizable_cards`（及 enabled/distribution/featured_card_ids）后，`config_panel.py:1268-1282` 遍历 `self._store.pools` 读 `pool.epitomizable_cards` 填充 targeted 家族『初始定轨』下拉无需改代码——但 Ph3 实施时须以 banner 模式 config 手工验证该下拉非空（定轨功能不静默退化）；若透传实现受阻则改在 §3.10.7 登记 `_populate_selected_card_combo` 改遍历 `store.banner.banners → pools[*]` 的适配**+ 展平视图 end_day=None 守卫消费端补列（REVIEW-R1-FIX: ISSUE-002）**：`core/worst_impact.py:390`（`_prepare_pool_info` 的 `pool_duration_days = pe.end_day - pe.start_day`，永久 Banner 展平后 `pe.end_day=None`、`None - int` 抛 TypeError）补 `end_day is None` 时 `pool_duration_days = 21` 兜底；`core/retreat_config.py:23`（`from_pool.end_day > 0`，退避点选永久池时 `None > 0` 抛 TypeError）补 `is not None` 守卫、`:47` 重建写侧 `end_day=p.end_day - offset_day` 对永久池同守卫（截断视图保持 None 或显式处理，随本阶段 `truncated.banner.banners` 迁移一并落地）；Ph9 增加「参考池/退避点为永久 Banner」回归用例**+ **cli.py:149 兜底目标推导适配（REVIEW-R1-FIX: ISSUE-003）**：兜底目标 id 改取 banner 段 `store.pools[0].pool_id.split('.')[0] + '_ssr'`（或改从 `store.banner` 推导），避免展平后全限定 pool_id 生成 `{banner}.main_ssr` 与真实卡 id 失配、CLI 无 `[[targets]]` 时 GDR 静默 0%（§3.9「cli.py 零改动」表述已修订为 pool_id 值级例外）+ **cli.py:77 `num_pools` 元数据语义登记（REVIEW-R1-FIX: ISSUE-325）**——展平视图下 `config_meta['num_pools'] = len(store.pools)` 由「池条目数」漂移为「跨 Banner 内层池总数」（多池 Banner 如 endfield_limited 展开为多个 PoolEntry），与旧 `[[pools]]` 模式输出不可直接对比：改补 `banner_count = len(store.banner.banners)` 输出或在 `-o` 输出中标注语义（`num_pools` 注明为内层池总数），不崩溃但保持展示可比**+ config_panel 写侧迁移（AUDIT-BREAK-1，从 Ph8c 前移，与 config_toml 写侧同 commit 约束）**：Ph3 属性化落地同 commit 迁移 `config_panel.py:3879`（apply_to_store `store.pools = []` 重绑 → AttributeError）、`:3385`（set_config append → 静默丢池）、`:3925`（apply_to_store append）三个写侧到 `store.banner`（BannerEntry/BannerPoolEntry，见 §3.9 写侧段）——或按 ISSUE-311 第二选项临时保留 `store._pools` 内部可变字段；Ph8c 仅保留 Tab UI 重写与读侧 Banner 适配**+ 测试侧 store.pools 赋值迁移（AUDIT-BREAK-3，随 {Ph3, Ph4} 交付单元落地）**：`tests/core/test_gdr.py:604 _make_store_for_p62`（:618 `store.pools = [...]` 赋值 → AttributeError；且该 P62 可达过滤 fixture 改从 `store.banner`（BannerEntry available_from/available_until + enabled）构建，否则 store.pools 空时目标卡整体误判不可达、`_obtainable` GDR 分母退化）、`tests/core/test_retreat_config.py:11 _make_store_with_3_pools`（:11 `store.pools = [...]` → 改从 `store.banner` 构建）、`tests/service/test_gacha_service.py:137`（`test_env_builder_from_config_store_smoke` 内 `store.pools = [...]` → 改从 `store.banner` 构建，与 Ph1b 已排的 pool_type 断言改写叠加）——三个 fixture 均改经 `store.banner.banners`（BannerEntry/BannerPoolEntry）构造、经展平视图消费，Ph9 补展平视图等价回归| 修改 | ~95行 |
| Ph4 <!-- REVIEW-FIX-PREV: AUDIT-BREAK-4 --> | `core/config_toml.py` —— `_build_banners()` 直接解析 `[[banner]]` + `[[banner.pool]]` + `[[banner.lifecycle]]` 段（**新建仅认 `[[banner]]` 的解析路径——现状解析器本无 `_wrap_pools_as_banners` 自动包装函数，非删除既有函数，D3/D4 裁决；rewards 统一 dict 表示，ISSUE-304，见 §3.4，REVIEW-R1-FIX: ISSUE-308**）+ **保存侧统一写 `[[banner]]`**（ISSUE-012，D3/D4 后简化）+ **max_draws 0→None 归一化（解析边界 `_build_banners`：TOML/UI 层「0=无限制」归一化为运行时 None 哨兵——`max_draws=0` 直接落引擎则 `_total_draws >= 0` 恒 True、首抽即自动 exhaust，与「0=无限制」语义相反，REVIEW-R1-FIX: ISSUE-331）**+ **pool_type/rerun_of 解析/写出残留清理**（ISSUE-012，含 `PoolEntry.pool_type`/`rerun_of` 字段删除）+ **一次性池语义固化（DECISION-1 归约：max_draws = batch_size 时 batch 抽完成后才耗尽）**（REVIEW-R1-FIX: ISSUE-002）+ **max_draws 非 batch_size 倍数批次中途耗尽语义固化**（耗尽标记在 draw 内、batch 循环 break 终止剩余，ISSUE-302）+ **Banner.max_draws 自动执行**（`_total_draws >= max_draws` → 自动 exhaust，新手池示例移除冗余 banner_draws 规则，ISSUE-303）+ **CLI `--migrate` 入口与旧格式残留警告处置（REVIEW-R1-FIX: ISSUE-004）**——`[[pools]]` 解析移除后：① cli.py:61-69 的 `--migrate` 依赖 `_build_pools`（config_toml.py:1026）与 `store._migrated_from_legacy`（P55 遗留标志），对经 `-c` 传入的旧格式配置将恒打印「TOML 已为新格式，无需迁移。」（`_migrated_from_legacy` 恒 False）且不做迁移——**删除/废弃 `--migrate` 入口**，或改为空 banner 时显式报错提示先手工迁移；② 非 migrate 路径旧格式经 `-c` 传入 → `load_toml` 得空 banner → `from_config_store` 产出 0 Banner → `end_time = max(...) if banners else 0` = 0 → `AllPoolsEndCondition(end_time=0)` 立即成立 → 模拟 0 抽、CLI 静默输出全 0——**`from_config_store`/`load_toml` 在 banner 为空且检测到旧 `[[pools]]` 段残留时给显式警告**而非静默 end_time=0（§3.4 已处理 from_config_store 空 banner 兜底，本条补 CLI 迁移入口的失效/误导行为）| 修改 | ~125行 |
| Ph4b | **配置一次性迁移（D3/D4 新增）** —— Ph4 解析器就绪后，把 `gacha_simulator/config/config.toml` 从 `[[pools]]` 改写为 `[[banner]]`（§3.4 迁移规则：一池一 banner、pool 进 `[[banner.pool]]`、start_day/end_day 上移、pool_type/is_rerun 删除、one_shot 改 max_draws=batch_size；**PityDef.pools 裸 id→全限定键改写已随原子提交落地，本阶段不重复，REVIEW-R1-FIX: ISSUE-103**）；**golden 等价对照改排 Ph6 之后（REVIEW-R1-FIX: ISSUE-102）**：改写动作本身按序执行（使 `store.banner` 有数据、Ph6 可验证），但「改写后用 Ph1-base golden 同种子重跑逐字段对比验证等价」须在 **Ph6 从 `store.banner` 恢复窗口之后**执行。Ph6 前 from_config_store 仍为 Ph1a 版、构造桥 None 窗口、逐池窗口过滤失效，此间重跑结果与 golden 的差异是中间态窗口缺失所致而非迁移行为差异，对照必然失败；Ph6 后以 banner 模式 from_config_store（窗口恢复）的运行结果为准逐字段对比，迁移后无任何 `[[pools]]` 段残留 | config.toml + tests | ~1h 手工 + 等价对照（Ph6 后） |
| Ph5 <!-- REVIEW-R1-FIX: AUDIT-BREAK-2 --> | `core/strategy.py` + `core/strategy_context_builder.py` —— `StrategyContext` 新增 `banners`/`all_banners` 字段，**且带默认值（`banners: List = field(default_factory=list)` / `all_banners` 同，AUDIT-BREAK-2）**——策略直接构造测试（test_worst_impact_correctness.py:73/102/130、test_strategy.py:11 `_make_ctx`、test_batch_draw.py:122）以 `current_pools=` 关键字构造 `StrategyContext` 且不传 banners，字段无默认值时 TypeError；**保留 `current_pools`/`all_pools` 仅遗留只读（由 `active_banners[*].active_pool` 推导填充、不删除——与 §3.4/§3.5 要点 8/§六 风险表/§七 验收的「保留」表述统一；D4 一次性迁移后 8 策略经 Ph1a 全迁移到 `banners`，`current_pools` 不再被内置策略依赖，但 `get_pity_probabilities`（strategy.py:109-121）遍历 `self.current_pools` 与 tests/core/test_p69_strategy.py:197-204/223 以 `current_pools=[]` 构造 `StrategyContext` 依赖其存在——删除分支下两者必然断裂且该测试不在 AUDIT-BREAK-7 清单内，故保留，REVIEW-R1-FIX: ISSUE-305）**；`build_strategy_context()` 新增 `banners`/`all_banners` 参数并透传（ISSUE-005），**两参数带默认值（`banners=None` 时置空列表，AUDIT-BREAK-2）——test_p69_strategy.py:223 `_min_strategy_context` 的 `build_strategy_context(**base)` 不传新参数，无默认值即 TypeError**。**`get_pity_probabilities(pool_id)` 的查询口径对齐（REVIEW-R1-FIX: ISSUE-305）**：保留分支下 `self.current_pools` 元素 `.id` 是 Banner 内字典键（'main'），无法承载全限定键——方法改经 `banners` 维度定位：`pool_id` 为全限定键 `{banner_id}.{pool_id}` 时拆分定位 `banner.active_pool` 读 rewards，再以全限定键调 `self._pity_engine.get_probabilities`（供 pity_reserve 等迁移后消费方使用，ISSUE-002）。**pity_reserve 若选 StrategyContext banner 维度 pity 概率接口方案（`get_pity_probabilities_for_banner(banner_id, pool_id)`，内部按全限定键查询 PityEngine），在此新增接口并透传（ISSUE-002，Ph1a 配合）**。与 P58 M4a 共用此函数签名，各自负责 `banners` / `_milestone_engine` 参数——**提前并入首提交（与 Ph1 同 commit 交付，REVIEW-R1-FIX: GATE-3，见 §3.12 交付单元注）**：`StrategyContext.banners` 字段与 `build_strategy_context` 的 `banners=` 参数是 Ph1a 策略迁移、Ph2 gacha_service 调用的前置依赖，必须随首提交落地，不可推迟到 Ph6 之后 | 修改 | ~16行 |
| Ph6 <!-- REVIEW-FIX-PREV: AUDIT-BREAK-5 --> | `service/batch_simulator.py` —— `SimulationEnv` 新增 `banner_defs` 字段（**带默认值，保证跨进程 pickle 兼容**，ISSUE-011）+ `from_config_store` 改从 `store.banner` 解析（start_day/end_day → Banner.available_from/until **经 `* DAY` 换算为秒**，ISSUE-001；**`available_until=None` 永久池兜底：逐 Banner 有效结束时间 `eff_end = until if until is not None else from + 21 * DAY`、`end_time = max(eff_end)`，与现状 `(start_day + 21) * DAY` 秒等价**——REVIEW-R1-FIX: ISSUE-007/001；删除对 `pe.start_day/end_day/pool_type/exchange_card_id` 的依赖，ISSUE-011；**注意 `exchange_card_id` 是『换源』而非删除（REVIEW-R1-FIX: ISSUE-313）**——从 `BannerPoolEntry.exchange_card_id` 透传构造 `Pool(exchange_card_id=...)`，运行时 Pool 保留该字段供 smart/pity_reserve/pool_quota/stop_on_target 4 策略 `pool.exchange_card_id == t.card_id` 定位兑换池（§3.4 保留清单），漏透传则 banner 模式兑换池匹配静默失效）+ **PoolPitySpec 在 banner 模式的 featured/ssr 来源 + 全限定键 `{banner_id}.{pool_id}`**（ISSUE-010/011）——`featured_ids` 由 banner 池 rewards 的 `featured=True` 标志聚合（`{r['card_id'] for r in rewards if r.get('featured')}`，替代 batch_simulator.py:517 的 `pe.featured_card_ids`，ISSUE-006）+ **Pool 构造时从 `rewards[].rarity` 回填 `Reward.extra_info['rarity']`（小写归一化，等价 batch_simulator.py:521-524 现状注入——`match='rarity'` 的 `_check_transitions` 唯一数据源，漏注入则 `extra_info['rarity']` KeyError 或恒空、新手池「出任意 6★ 即关闭」静默失效，ISSUE-007）** + **`_build_pity_engine_from_gui` 的 pool_specs fnmatch（batch_simulator.py:139-158）按全限定键匹配（D4 一次性迁移后 PityDef.pools 均为全限定键、无三路兼容；旧裸 id 已在 Ph4b 配置迁移改写，ISSUE-005；pool_specs 键全限定已前移至原子提交随 Ph2 落地，AUDIT-BREAK-3/§3.5 要点 12——Ph6 保留数据源改 store.banner + 多池展开；**`_build_pity_engine_from_gui` 遍历对象重写显式落地（REVIEW-R1-FIX: ISSUE-332）**：该函数体（batch_simulator.py:139-158）的 `for pool in pools:` 在 Ph6 `SimulationEnv.pools` 承载 `List[Banner]` 后对元素读 `pool.rewards` 抛 AttributeError——改为 `for b in pools: for p in b.pools.values():` 双重展开、`compute_scope_mappings(p)`（pity.py:1070 读 `pool.rewards`）以展开后的 Pool 为参、featured/ssr 来源从 `p.rewards` 聚合、`pool_specs` 键按 `f"{b.id}.{pools 字典键}"` 全限定拼接（§3.5 要点 11 同口径）——多池展开后键自然承接原子提交态的 `{pid}.main` 全限定键，不重复实现 fnmatch）** + TargetCard.pool_ids 标识规则钉死为 banner 级（REVIEW-R1-FIX: ISSUE-315，替代 ISSUE-011 原二义「banner 级或全限定」）：`TargetCard.pool_ids` 一律为 **banner 级键**——batch_simulator 构造 TargetCard（:188 `_wk_init` / :300 run_batch_parallel）时对 `card_defs[idx]['pools']` 各键取 banner_id 段（`k.split('.')[0]`，无 `.` 段则原样保留），而非全限定键。理由：Ph3 后 card_defs pools 为全限定键（§3.12 Ph3 / ISSUE-310），若不取段则 `TargetCard.pool_ids = ['pool_c1.main']` 而 4 策略 `_pool_needs_target` 以 `banner.id`（'pool_c1'）匹配（§3.4 ISSUE-303）→ `'pool_c1' in ['pool_c1.main']` 恒 False、静默跳过目标追卡；取段后 `['pool_c1']` 与 `banner.id` 恒同口径。`_ensure_pool_to_targets` 缓存键随之以 banner.id 段对齐；Ph9 的 4 策略目标追卡回归（ISSUE-303/315）锁定该规则+ PoolSchedule 时间源改 Banner 级 + **`env.pools` 运行时消费端迁移（AUDIT-BREAK-5——`SimulationEnv.pools` 改承载 `List[Banner]` 后，以下消费端对元素读裸 Pool 属性必然 AttributeError，原计划未纳入任何阶段）**：① `batch_simulator.py:676` 的 `all_drawable_ids = [r.id for p in pools for r, _ in p.rewards]`——改为遍历 banner 池展开（`b.pools[*].rewards` 或 `store.pools` 只读视图）；② `core/retreat_search.py:407-412` `_get_obtainable_card_ids`（`pool.is_exchange`/`pool.exchange_card_id`/`pool.rewards`）及同文件多处 `if not env.pools`/`for pool in env.pools`——改为按 banner 池展开读（`banner.pools.values()`）；③ `gui/resource_search_panel.py:61` `_extract_cost_per_draw(self._sim_env.pools)` 读 `p.cost`——改为 `banner.active_pool.cost` 或展平读取；④ **`core/retreat_search.py:718-738` 模块级 `get_cost_per_draw`（被 `PlanSearchEngine._extract_cost_per_draw` :219 调用，返回值用于 `epsilon = cost_per_draw * precision_draws` :320 二分收敛阈值与 `result.cost_per_draw` 报告）——`getattr(p, 'cost', None)` 防御式读取在 `List[Banner]` 上返回 None → `continue` → **静默回退 160**（非「必然 AttributeError」崩溃型，故不在原 AUDIT-BREAK-5 清单内、实施时易漏改）——改为 `banner.active_pool.cost` 或展平读取（REVIEW-R1-FIX: ISSUE-304）**；Ph9 补充一条非 160 成本配置（如 orundum:600）的 `cost_per_draw` 回归断言（§3.12 Ph9）**+ Banner 运行时状态跨模拟隔离（REVIEW-R1-FIX: ISSUE-312，阻塞）**：Ph6 后 `SimulationEnv.pools` 承载 `List[Banner]`、`_run_single`（batch_simulator.py:209）每次从 env.pools 构造 GachaService，构造桥（§3.5 要点 10）对 Banner 元素是『直接收纳 `{b.id: b}`』的**引用**而非拷贝——Banner._pool_draws/_total_draws/_exhausted_pools/_active_pool_id 在 draw/_check_transitions 期间被修改，而单进程路径（max_workers<=1 直连 `_run_single`）与 worker 内 `_wk_env` 均**跨多次模拟复用同一 env**：第 N 次模拟的池抽数/耗尽标记/激活池原样泄漏到第 N+1 次，固定种子不可复现（现状 Pool 仅有 `_adjusted_cum_weights` 且 draw 后自行复位、PityState/GachaState 每次重建，均无此泄漏；P61 打破该隔离前提）。处置：**每次 `_run_single` 构造 GachaService 前对 env.pools 深拷贝（`copy.deepcopy`，或 Banner 实现 `clone()`/`reset()`，或从 `env.banner_defs` 重建 `List[Banner]`）**；Ph9 新增『同 env 连续多次 `_run_single`（固定种子）结果逐字段可复现』的批量回归用例**+ `env.pool_end_times` 键变化 GUI 消费链登记（REVIEW-R1-FIX: ISSUE-006）**——batch_simulator.py:677 `pool_end_times = {s.pool_id: s.available_until for s in schedules}` 由 PoolSchedule 构建、键为裸 pool_id，Ph6 时间源改 Banner 级后键变 banner_id。下游 GUI 消费链须一并登记并明确展示语义（不得依赖「消费方零改动」笼统表述）：`main_window.py:339/350`（转发）、`process_analysis_panel.update_results(pool_end_times=...)`（:291-299）、`analysis_panel.py:1260-1261/1336-1346`（池结束时间图表，`sorted(self.pool_end_times.items(), key=lambda x: x[1])`）、`data_manager_panel.py:396`（数据集详情展示）拿到 banner 级键——逐池累计分析/展示在 UI 层按 banner 键展示或按全限定键拆分展示（与 §3.5 要点 6 `banner_end_times_sorted` 同键语义，Ph7 统计层与 GUI 展示层同步，见 §四 波及范围）| 修改 | ~85行 |
| Ph7 <!-- REVIEW-FIX-PREV: AUDIT-BREAK-6 --> | 统计层适配 —— GDR / 过程分析 / 流式分析以 Banner 为聚合单位 + `CompactResult.pool_types` 由 output/random 推导填充（ISSUE-004）+ **逐池统计键/`pool_types` 键迁移到全限定 `{banner_id}.{pool_id}`**（gdr/process_trace/main_window/data_manager 消费端同步，ISSUE-002）+ `pool_end_resources`/`pool_end_pity_states` 消费端迁移到 banner_end（ISSUE-003，含 `core/vulnerability.py` 逐池分箱消费——REVIEW-R1-FIX: ISSUE-005，**含 `gui/gacha_panel.py:111` 的 `no_draw_results[0].get('pool_end_resources', {})`——字段改名后 `.get` 静默拿空 dict 无异常，须改 `.get('banner_end_resources', {})`，AUDIT-BREAK-6**；**含 `core/streaming.py` 生产端 `compact.get('pool_end_resources', {})` 读点（REVIEW-R1-FIX: ISSUE-322）——extract_aggregate（:114-115）/ DrawSequenceExtractor（:265/:284）/ WorkerLocalExtractor._update_cumulative（:525/:533 写累积快照 pool_end_resource(s) 字段）——字段改名后 `.get` 静默拿空 dict、`compute_gdr_from_cumulative`（gdr.py:1115-1121）回退 initial_resources/derive 推算分支、per-pool 累计 GDR 与 P62 可达过滤伪最终资源静默退化，须改 `.get('banner_end_resources', {})` 且键随 `pool_end_times`（batch_simulator.py:677，Ph6 已改 banner 级）同步改 banner 级，实施时核对与 gdr.py:1115 消费链的数据源一致**；**含 no_draw 路径 `banner_end_resources` 键口径裁定（REVIEW-R1-FIX: ISSUE-323）——no_draw 模拟经 `on_banner_end(banner_id)`（§3.5 要点 6）写快照、键为 **banner 级**，而 `analysis_panel.py:1132` `no_draw_pool_resources.get(pid, {})` 的 pid 来自 `all_pool_ids`（:1037，全限定 `{banner_id}.{pool_id}` 键）恒 miss、`vulnerability.py:941-942` 山脊线基线同理——`analysis_panel.py:1129-1137` 与 `retreat_panel.py:96` 两下游消费端登记：pid 取 banner_id 段（`k.split('.')[0]`）或按 banner 段取资源基线，`resource_remaining` 每池 GDR 基线不静默缺失，Ph9 加 no_draw + 多池 Banner 回归**）+ **cli.py:212-213 `cumulative_snapshots_pools` 键列表展示口径登记（REVIEW-R1-FIX: ISSUE-325）**+ **可比性指纹/config_hash 纳入 Banner 级配置**（ISSUE-013）| 修改 | ~80行 |
...
| Ph8b | `gui/config_panel.py` ——「保底机制」Tab 增强：删除手写 `pools` 文本框 → 改为「绑定池」勾选表格（~60行；变更总览不含不存在的「生效范围 QGroupBox」「快速绑定 QDialog」，ISSUE-015）+ **空勾选保存语义（全不选 → `pools=[]` = 不绑定任何池，保存分支显式区分「空勾选 → []」与「全选 → `*`」，不沿用旧「空文本 → `('*',)`」逻辑，REVIEW-R1-FIX: ISSUE-328）** | 修改 | ~60行 |
<!-- REVIEW-R1-FIX: AUDIT-BREAK-1 -->
| Ph8c | `gui/config_panel.py` —— `apply_to_store()`/`set_config()`/`get_config()` Banner 适配 + 移除池子模板 + `_setup_ui()` 注册 Tab + **`store.pools` 写侧迁移到 `store.banner` 已前移至 {Ph3, Ph4} 交付单元（AUDIT-BREAK-1：:3879 重绑 / :3385/:3925 append 在 Ph3 属性化后立即 AttributeError/静默丢池，不得等到 Ph8c；本阶段仅保留 Tab UI 重写与读侧 Banner 适配，写侧迁移见 §3.12 Ph3）** + **`gui/main_window.py:276-277` 内联分布提示删除/改写**（§3.10.6 移除模板后 `not p.distribution_template` 恒 True、状态栏提示恒显示，REVIEW-R1-FIX: ISSUE-005） | 修改 | ~45行 |
| Ph9 | `tests/test_banner.py` —— 覆盖 lifecycle 全部规则 + 送抽 + step + 新手池 + 配置一次性迁移 + 基线固化等价对照 + **策略迁移回归**（8 策略在 banner 模式跑通，ISSUE-001）+ **pool_quota / pity_reserve 在 banner 模式的等价回归**（配额判定与保底阈值不静默退化——`ctx.pool_draw_counts` 裸键、`get_pity_probabilities` 裸键全限定化后不再恒 0 / 回退基础概率，ISSUE-002）+ **多 Banner 同名池配额回归**（pool_quotas 全限定键下同名 'main' 配额不串池，REVIEW-R1-FIX: ISSUE-327）+ **draw_target `pool_id` 匹配回归**（非空 pool_id 配 `banner.id`、banner 模式下命中不恒 WaitAction——worst_impact 唯一消费方不退化，REVIEW-R1-FIX: ISSUE-326）+ **「保底机制」Tab 空勾选保存回归**（全不选保存 `pools=[]`、加载不勾选任何行、保底规则不触发；全选保存 `*`、加载全勾选，REVIEW-R1-FIX: ISSUE-328）+ **no_draw + 多池 Banner 的 `resource_remaining` 回归**（banner_end_resources 键为 banner 级、analysis_panel/vulnerability 按 banner_id 段取基线不 miss，REVIEW-R1-FIX: ISSUE-323）+ **时间窗口单位等价用例**（21 天开池在 `real_time >= 21*DAY` 秒时开放/关闭、`on_banner_end` 在第 21 天而非第 21 秒触发、`AllPoolsEndCondition` 秒判定，ISSUE-001）+ **无抽卡跨 time_window 用例**（等待分支触发 time_window 转换、目标池可达不死锁，ISSUE-003）+ **`_build_pity_engine_from_gui` 全限定键保底绑定用例（迁移后 `pools=["genshin_limited.main"]` 精确命中）**（ISSUE-005）+ **显式 pool_id 派发用例**（`banner_id=None` + 裸 `pool_id` 反查唯一 Banner 单池路径；显式双字段时派发恒走 `banner.active_pool`、显式 pool_id 不改变池选择，REVIEW-R1-FIX: ISSUE-316）+ **card_obtained rarity 匹配用例**（终末地新手池「出任意 6★ 即关闭」——覆盖 banner 模式 Pool 经 Ph6 rarity 回填的路径，ISSUE-306/007）+ **max_draws 非 batch_size 倍数批次中途耗尽用例**（如 15/10，总抽数恒 15，ISSUE-302）+ **pending_transitions 事件型 remaining=-1 策略守卫用例**（ISSUE-305，调用签名 `pending_transitions(ctx.state.real_time)`——REVIEW-R1-FIX: ISSUE-330）+ **time_window remaining 单位断言**（2.5 天窗口：`(at_value - real_time)/DAY` 后向上取整得 3 而非秒值 216000，垫刀判据不差 86400 倍，REVIEW-R1-FIX: ISSUE-317）+ **4 策略 `_pool_needs_target` 目标追卡回归**（smart / pity_reserve / pool_quota / stop_on_target 在 banner 模式以 `banner.id` 命中 `TargetCard.pool_ids`——目标卡未达成时策略实际抽卡而非只等待，REVIEW-R1-FIX: ISSUE-303/315）+ **4 策略兑换池定位回归**（banner 模式 `exchange_card_id` 经 Ph6 透传后 `pool.is_exchange and pool.exchange_card_id == t.card_id` 命中、兑换池不因字段缺失 AttributeError 或匹配静默失效，REVIEW-R1-FIX: ISSUE-313）+ **批量多次模拟可复现回归**（同 env 连续 `_run_single` 固定种子结果逐字段一致，Banner 运行时状态不跨模拟泄漏，REVIEW-R1-FIX: ISSUE-312）+ **最差影响分析保底触发回归**（`worst_impact._build_pity_engine` pool_specs 键为 `{pid}.main` 时 `before_draw` 保底调整生效、soft pity 不被静默跳过，REVIEW-R1-FIX: ISSUE-301）+ **最差影响分析 `analyze()` 级 `expected_pools` 回归**（99 池错峰窗口保留、连续池目标可达成、`expected_pools ≈ MAX_POOLS` 不退化，REVIEW-R1-FIX: ISSUE-302）+ **`cost_per_draw` 非 160 成本回归**（orundum:600 配置经 `PlanSearchEngine._extract_cost_per_draw` → `get_cost_per_draw` 返回 600 而非静默回退 160，REVIEW-R1-FIX: ISSUE-304）+ **窗口关闭/未开后策略不可再抽该 Banner 用例**（ISSUE-001）——banner 配置 `available_from`/`available_until`：`real_time < available_from`（未开）与 `real_time > available_until`（已关）时 `banner.is_available(real_time)` 为 False、不进入 `active_banners`、策略 select_action 不返回该 banner 的 DrawAction；等待期 `real_time` 越过 `available_until` 触发 `on_banner_end` 后 `banner.is_exhausted` 为 True（关闭标记生效）；等待期越过 `available_from` 后窗口未开 banner 重新可抽 + **参考池/退避点为永久 Banner 回归用例**（ISSUE-002）——worst_impact 配置含 `available_until=None` 永久 Banner 时 `_prepare_pool_info` 不抛 TypeError（`end_day` None → `pool_duration_days=21` 兜底）；retreat_config 退避点选永久池时 `offset_day` 不抛 TypeError（`is not None` 守卫、兜底 `start_day + 21`）、重建写侧对永久池 `end_day=None - offset_day` 不崩（截断视图保持 None） + **CLI 无 `[[targets]]` 兜底目标回归**（ISSUE-003）——banner 模式展平视图下 cli.py 兜底目标 id 取 banner 段、GDR 非 0%（配置无 [[targets]] 时 ssr 计数不恒 0）+ **CLI 旧格式 `-c` 与 `--migrate` 处置回归**（ISSUE-004）——旧 `[[pools]]` 格式经 `-c` 传入时 `--migrate` 不打印「已为新格式」误导信息（或入口已删除）、空 banner 时 from_config_store/load_toml 给显式警告而非静默 end_time=0/模拟 0 抽 + **max_draws=0 无限制配置不自动 exhaust 用例**（TOML `max_draws=0`（§3.6 合法无限制写法）经解析边界归一化为 None 后引擎硬上限判定短路（`max_draws is not None` 守卫不成立）、Banner 不自动 exhaust、抽数不受硬上限限制；UI QSpinBox 0 保存同样经保存边界归一化，REVIEW-R1-FIX: ISSUE-331） | 新建/修改 | ~260行 |
...
<!-- REVIEW-FIX-PREV: GATE-3 -->
<!-- REVIEW-FIX-PREV: GATE-3_依赖顺序 -->
<!-- REVIEW-FIX-PREV: GATE-6_测试策略 -->
...
> 3. **Ph2 依赖 Ph1c**：`run_simulation` 的 banner 路由（`banners.get(action.banner_id)` / `banner.draw(...)` / `banner.active_pool` / `banner.is_available(state.real_time)`——带参方法，ISSUE-001）全部消费 `core/banner.py` 的 `Banner` 类，无 Ph1c 则 Ph2 无对象可路由。
...
>
<!-- REVIEW-R1-FIX: AUDIT-BREAK-2/3 -->
> **结论**：Ph1、Ph1a、Ph1b、Ph1c、Ph2、Ph5 **六阶段一次 commit 落地**（内部仍按表序实施、允许分步调试，但不得单独提交/发布中间态）；首提交后**现有 `pytest` 全量通过**——判据含 `test_state` 删用例与 `test_env_builder_from_config_store_smoke` 改写随原子单元同步落地、`GachaService` 构造桥使 `_run_single` 与直接构造的测试均有 `self._banners` 可用；**既有测试破坏面远大于上述两项（REVIEW-R1-FIX: ISSUE-022/AUDIT-BREAK-7）——以下既有测试以将删字段为关键字构造 `Pool(...)`/`PoolEntry(...)` 或直测将删 API，随 Ph1/Ph4 必断，须全部并入原子单元对应阶段，否则『首提交后 pytest 全量通过』不成立**：① `tests/core/test_pool.py:36-41`（`Pool(available_from=0, available_until=100)` + 直测已退役的 `pool.is_available_at()`，Ph1 删字段+退役方法即双断——用例删除或改断言推导属性）；② `tests/service/test_gacha_service.py:14-21` `_make_pool`（`available_from=0.0, available_until=...` 关键字，被 :32/51/78/101/121 等大量用例共用，Ph1 即断）与 :68-69（`test_initial_count_multiple_cards` 直接 `Pool(... available_from/available_until)`）；③ `tests/core/test_epitomizable_cards.py:189/202-205`（`Pool(pool_type='武器'/'角色', available_from=0, available_until=21)`）；④ `tests/core/test_pity_integration.py:97/150`（`Pool(pool_type='角色')`）；⑤ `tests/core/test_batch_draw.py:24` 与 `tests/service/test_gacha_service.py:142`（`PoolEntry(... pool_type='角色')`，随 Ph4 删 `PoolEntry.pool_type` 断——test_batch_draw 的该处随 Ph4 移除关键字，test_gacha_service 的随 Ph1b 一并移除，AUDIT-BREAK-2/4）。**策略层直接构造测试破坏面（AUDIT-BREAK-2，4a transitive_gaps 独立验证补充——不在原 AUDIT-BREAK-7 清单内，须随原子单元改写，否则 Ph1a 落地后 `pytest` 必红）**：⑥ `tests/core/test_worst_impact_correctness.py:66/94/123` 三个 draw_target 测试（StrategyContext 构造在 :73/102/130）以 `_FakePool`（:14，无 banner.is_available 接口）经 `current_pools=` 构造 + 断言 `action.pool_id` 裸键——Ph1a 迁移 draw_target 遍历 `ctx.banners` 后 `ctx.banners`（默认空）恒空、策略恒 WaitAction、断言 `action.pool_id` 对 WaitAction 抛 AttributeError——改经 Banner 对象构造 ctx.banners（或构造 `Banner(id=..., pools={'main': pool})` 包装）并断言 `action.banner_id`/全限定键；⑦ `tests/core/test_strategy.py:11` `_make_ctx`（`StrategyContext(current_pools=...)` 不传 banners——字段无默认值即 TypeError，默认值兜底后 test_target_hunting_strategy 仍断言 `action.pool_id == 'standard'`，Ph1a 后 target_hunting 读 `ctx.banners`、空列表恒 WaitAction 断言失败——改传 banners 包装）；⑧ `tests/core/test_batch_draw.py:122`（SmartStrategy 测试 `StrategyContext(current_pools=[pool])` 构造）与 `:130`（断言 `len(call_args) >= 1` 依赖 current_pools 遍历，Ph1a 后 SmartStrategy 读 `ctx.banners` 空 → call_args 空断言必失败——改传 `banners=[Banner(id=..., pools={'main': pool})]` 包装）；⑨ `tests/core/test_p69_strategy.py:223`（`_min_strategy_context` 的 `build_strategy_context(**base)` 不传 banners/all_banners——两新参数须带默认值，见 Ph5 行）。**测试侧 store.pools 赋值破坏面（AUDIT-BREAK-3，随 {Ph3, Ph4} 交付单元落地——Ph3 属性化后 `store.pools = [...]` 对无 setter property 赋值 AttributeError / 静默丢池，AUDIT-BREAK-7 漏列）**：⑩ `tests/core/test_gdr.py:604 _make_store_for_p62`（:618 赋值 → 改从 `store.banner` 构建，且 store.banner 空时 P62 可达过滤目标卡整体误判不可达、`_obtainable` GDR 分母退化）；⑪ `tests/core/test_retreat_config.py:11 _make_store_with_3_pools`（赋值 → 改从 `store.banner` 构建）；⑫ `tests/service/test_gacha_service.py:137`（`test_env_builder_from_config_store_smoke` 内赋值 → 改从 `store.banner` 构建，与 Ph1b 已排改写叠加）。**另补（AUDIT-BREAK-3）：`tests/core/test_pool.py:24`（`test_exchange_pool_returns_first_reward` 以 `is_exchange=True` 关键字构造 `Pool`——Ph1 将 is_exchange 改推导 property 后 TypeError；改移除关键字，rewards=[(r, 1.0)] 单卡 100% 分布推导 `is_exchange=True` 语义不变，AUDIT-BREAK-7 漏列）**。**注**：`tests/core/test_pity_config_toml.py:32/266` 的 `pool_type = "武器"` 是 TOML 配置字符串（旧键解析时忽略、不报错），无需改动；`tests/core/test_schedule.py:5-7` 测的是 `PoolSchedule.is_available_at`（非 `Pool`，PoolSchedule 保留），不受影响；`tests/core/test_process_analysis.py:26` 的 `pool_type='draw'` 是 `PoolEvent` 字段（非 `Pool.pool_type`），不受影响。；Ph9 `test_banner.py` 的 8 策略 banner 模式回归属**新增用例**、在 Ph9 交付，不构成首提交的通过判据。Ph1-base（基线固化）在首提交前独立交付（用旧代码 + `[[pools]]` 配置生成 golden fixture，不参与原子单元，其失败不代表代码断裂）；Ph3/Ph4/Ph4b/Ph6/Ph7/Ph8/8b/8c/Ph9 依序独立交付，各自保持可运行态。**例外：{Ph3, Ph4} 为同一交付单元（REVIEW-R1-FIX: ISSUE-311）**——Ph3 把 `store.pools` 属性化为只读展平视图、Ph4 才移除 `_build_pools` 的写侧（config_toml.py:1107 `store.pools.append(pool_entry)`），两者落地之间 `load_toml`（:78 调用链）仍执行 `_build_pools` 对只读属性返回的派生列表 append——不抛异常、静默丢池（load_toml 后 `store.pools` 为空），随后 `_backfill_card_pools` 遍历空列表使 `card_defs[].pools` 全空，下游 GDR/GUI/模拟拿到空数据且无异常。处置：**Ph3 落地时必须同步移除/停用 `_build_pools` 写侧（与 Ph4 同 commit）**，或 Ph3 阶段临时保留可变字段路径（`store._pools` 内部字段）、Ph4 一次性切换为纯展平视图——二选一，不得让『Ph3 独立提交、Ph4 后补』的中间态存在。**config_panel 写侧（`config_panel.py:3879/:3385/:3925`）受同 commit 约束（AUDIT-BREAK-1，从 Ph8c 前移）——属性化后对无 setter property 赋值 AttributeError/append 静默丢池，与 config_toml 写侧一并随 {Ph3, Ph4} 迁移到 store.banner（见 §3.9 写侧段 / §3.12 Ph3）**。§5.3 实施顺序与 §六 风险表同口径更新。
>
> **GATE-3 验收范围收缩（REVIEW-R1-FIX: ISSUE-102）**：『首提交后现有 `[[pools]]` 行为完全不变』限定为「**非时间窗口相关行为不变**」。原子提交态（Ph1-Ph5 落地、Ph6 未落地）构造桥包装 Banner 窗口为 None（§3.5 要点 10），`banner.is_available(state.real_time)` 恒 True（available_from/until 均 None，时间比较恒真），**逐池时间窗口抽取可用性过滤不生效**（旧模型 `current_pools` 按 `available_from`/`available_until` 过滤，gacha_service.py:224-226）。样本 config 错峰池在中间态全被抽取属**已声明分歧**，不影响首提交可运行与 pytest 全量通过（测试配置单池无错峰窗口、pytest/golden 不暴露）；逐池窗口过滤等价性由 **Ph6 后**（from_config_store 从 store.banner 恢复窗口）的 Ph1-base golden 等价对照锁定（§3.12 Ph4b 已改排 Ph6 后）。
<!-- REVIEW-R1-FIX: ISSUE-102 -->
§六 风险表首行与 §5.3 实施顺序以本注为准。

<!-- REVIEW-FIX-PREV: GATE-4 -->
...
- **两者都是推导 property，非配置字段，从 rewards 计算**：
  ...
  - `random = bool(rewards) and (len(rewards) > 1 or rewards[0][1] < 1.0)`——**tuple 表示**（运行时 `Pool.rewards: List[Tuple[Reward, float]]`，pool.py:146，概率是元组第二元素，`Reward` 无 `.prob` 属性）；**dict 表示**（`BannerPoolEntry.rewards: List[dict]`，§3.9）时取 `rewards[0]['probability'] < 100.0`（概率%）。两种表示分别实现，避免照抄 `rewards[0].prob` 抛 AttributeError（REVIEW-R1-FIX: ISSUE-008）。**tuple 表示假定概率已归一化 0-1（REVIEW-R1-FIX: ISSUE-319）**——from_config_store 以 `de.probability / 100.0` 归一化，直构 Pool 的调用方（含 worst_impact，Ph1a）必须同口径 `/ 100.0`，否则单卡 100% 池 `100.0 < 1.0`=False → random=False → is_exchange=True 被误判为兑换池（draw() 走 exchange 分支恒返回首卡）；概率已归一化；单卡 100% = 确定性；空 rewards 显式短路为 False，避免 `rewards[0]` 对空列表索引抛 IndexError（REVIEW-R1-FIX: ISSUE-010）
- 组合语义：
...
- 消费点修正：
  ...
  - **兼容模式键格式（DECISION-4 已裁决，2026-08-02，见 §6.1）**：**一次性迁移到全限定键 `{banner_id}.{pool_id}`，不保留旧裸键、不做旧数据迁移映射（砍兼容）**（REVIEW-R1-FIX: ISSUE-318 表述清理——此前「默认选型：全限定键 + 消费端兼容读取（旧数据集经映射迁移，Ph7 一并处理）」「保守替代方案：兼容模式统计键保持裸 `{旧pool_id}`」的残留表述与 DECISION-4 砍兼容裁决直接冲突，删除）。本项目**未上线、无历史已保存 compact 结果**（CLAUDE.md「无历史包袱原则」，见 DECISION-4 决策依据），旧 `[[pools]]` 配置仅允许一次性改写为 `[[banner]]`（等价性由 Ph1-base 基线固化等价对照验证）；新代码仅一种键格式（全限定 `{banner_id}.{pool_id}`）、无 `{旧pool_id}.main` 退化情形；可比性指纹 / `config_hash` 版本化直接按 banner 维度（ISSUE-013，Ph7），无旧数据集键映射路径
...
- **`pool_start` 键格式钉死为全限定键（REVIEW-R1-FIX: ISSUE-310）**：banner 模式下构建 `pool_start` 时**键 = 全限定 `{banner_id}.{pool_id}`**（遍历 `store.banner.banners` → 对每个 banner 的**每个 pool** 展开，`key = f"{banner.id}.{pool.id}"`、`value = banner.available_from`（秒）），**不是 banner_id**。理由：gdr.py:508 的 `card_pools[cd.card_id] = set(cd.pools)` 消费 `card_defs[].pools`——该集合由 `config_toml._backfill_card_pools()`（config_toml.py:1178-1184）遍历 `store.pools` 写入 `pool.pool_id` 回填，Ph3 属性化后 `store.pools` 为展平视图、pool_id 即全限定键，故 `card_pools` 天然是全限定键空间。若 `pool_start` 以 banner_id 为键，`pool_start.get(pid)` 恒 None、4 个 `_obtainable` GDR 分母静默退化，与 §七 验收『banner 模式下 `_obtainable` GDR 分母语义一致』冲突。**`_backfill_card_pools` 的 Ph3 适配**：该函数遍历对象随 `store.pools` 属性化变为展平视图、写入的 pool_id 即全限定键，与 `pool_start` 同口径——**无需改写入规则**，但须在 Ph3 实施清单显式登记该键空间变化（`card_defs[].pools` 由裸 id → 全限定键），gdr.py 的 `card_pools` 与 `pool_start` 键空间保持一致；batch_simulator.py:656-662 的 card_defs pools 回填（遍历 pool_entries 同样取 `pe.pool_id`）在 Ph3 后同为全限定键，同口径
- **比较单位修正声明（REVIEW-R1-FIX: ISSUE-307）**：现状 `pool_start[p.pool_id] = p.start_day`（天）与 `final_time`（秒）直接比较——对 `final_time ≥ 86400` 的常态场景近似恒真可达（天 vs 秒的单位 bug，晚开池也被误判可达）。改读 `banner.available_from`（秒）后比较变为秒 vs 秒、语义正确——**晚开池（如样本 pool_c8 day 147）在早停/截断时间线场景（`final_time` < 该池 open 时刻）由「可达」变「不可达」**，4 个 `_obtainable` GDR 分母随之收严。此修正符合 P62 可达过滤语义（分母仅含模拟期间池子已开放的目标卡），属**有意修正**而非回归；Ph1-base golden 用全时间线配置、不暴露此差异，Ph9 补充晚开池早停/截断场景的 `_obtainable` GDR 用例锁定新分母，验收措辞相应更新（见 §七）
...
| `core/strategy.py` | `StrategyContext` 新增 `banners` + `all_banners` 字段 + **保留 `current_pools`/`all_pools` 仅遗留只读（由 `active_banners[*].active_pool` 推导填充，不删除）+ `get_pity_probabilities` 改经 banners 维度定位全限定键（REVIEW-R1-FIX: ISSUE-305）** | +5行 |
| `core/strategy_context_builder.py` | `build_strategy_context()` 新增 `banners`/`all_banners` 参数并透传（ISSUE-005）——从「不触及」移入 | +6行 |
| `strategies/builtin/*.py`（8 个内置策略） | 从 `pool.is_available_at`/`pool.available_until` 迁移到 `ctx.banners` + `DrawAction` 双字段（ISSUE-001/006）——从「策略无需修改」移入**强制适配**（Ph1a）；含 fixed_count / target_hunting（原「可不迁移」取消——Ph6 多池 banner 模式下 `current_pools` 元素 id 为 pools 字典键、非全局唯一（原子提交态为旧裸池 id，理由以 Ph6 后口径为准），`target_pool_ids` 匹配改 `banner_id`，REVIEW-R1-FIX: ISSUE-003/321）；**含 pool_quota（`ctx.pool_draw_counts` 裸键→`banner.pool_draws`）/ pity_reserve（`get_pity_probabilities` 裸键→全限定或 banner 维度接口）两个消费点（ISSUE-002）**；**含 4 策略 `_pool_needs_target` 匹配参数改 `banner.id`（smart.py:61 / pity_reserve.py:43 / pool_quota.py:49 / stop_on_target.py:52，与 `TargetCard.pool_ids`——钉死为 banner 级键、batch_simulator 取全限定键 banner_id 段——对齐，REVIEW-R1-FIX: ISSUE-303/315）** | ~95行变更 |
...
| `service/batch_simulator.py` <!-- REVIEW-FIX-PREV: AUDIT-BREAK-5 --> | `SimulationEnv` 新增 `banner_defs`（带默认值）**与 `milestone_engine`（REVIEW-R1-FIX: ISSUE-329，P58 engine 跨进程传输载体——`_run_single` 装配前从 env 取出注入 `register_milestone_engine`，带默认值 None 保 pickle 兼容，Ph0）** + `from_config_store` 改读 `store.banner`（start_day/end_day *DAY 换算秒）+ 全限定 pity 键 + `_build_pity_engine_from_gui` 全限定键匹配（ISSUE-005，D4 一次性迁移后无三路兼容）+ **banner 模式 Pool 从 `rewards[].rarity` 回填 `extra_info['rarity']`**（ISSUE-007）+ TargetCard.pool_ids 规则（ISSUE-010/011）+ **Notifier 装配点（`_run_single` 创建+注入 GachaService、构造前回调 P58 装配函数，REVIEW-R1-FIX: ISSUE-301）** + **Ph1a 构造点迁移（`from_config_store` :547-559 删 `available_from`/`available_until`/`pool_type`/`is_exchange` 四个关键字，`exchange_card_id` 保留、时间窗口改由 PoolSchedule 承载，REVIEW-R1-FIX: GATE-3_依赖顺序；Ph6 将 `exchange_card_id` 数据源换为 `BannerPoolEntry.exchange_card_id`，ISSUE-313）** + **`all_drawable_ids` 构建（:676 读 `p.rewards`）改 banner 池展开（AUDIT-BREAK-5，Ph6）** + **每次 `_run_single` 构造 GachaService 前深拷贝 env.pools（Banner 运行时状态跨模拟隔离，否则多次模拟间 `_pool_draws`/`_exhausted_pools`/`_active_pool_id` 泄漏、固定种子不可复现，REVIEW-R1-FIX: ISSUE-312，Ph6）** | ~85行 |
| `service/config_service.py` <!-- REVIEW-FIX-PREV: AUDIT-BREAK-1 --> | `Pool(...)` 关键字构造点迁移（ISSUE-008）+ **`export_pool_to_config`（:36-37 读 `p.available_from`/`p.available_until`）与 `import_pool_from_config`（:52-60 以将删字段构造 `Pool(...)`）两方法为死代码（无调用方，AUDIT-BREAK-1）——随 Ph1a 直接删除** | ~15行变更 |
| `core/config_toml.py` | `_build_banners()` 直接解析 `[[banner]]`（新建仅认 `[[banner]]` 的解析路径，现状本无 `_wrap_pools_as_banners` 自动包装，REVIEW-R1-FIX: ISSUE-308）+ 保存侧统一写 `[[banner]]` + 配置一次性迁移 + pool_type/rerun_of 残留清理（ISSUE-012）| ~110行 |
| `core/worst_impact.py` | `Pool(...)` 关键字构造点迁移（ISSUE-008）+ **99 池错峰窗口改直接构造带 `available_from`/`available_until` 的 Banner（from_dict 直构 `List[Banner]`，不经 Ph6 from_config_store，REVIEW-R1-FIX: ISSUE-302）+ `_build_pity_engine` pool_specs 键改全限定 `{pid}.main`（随原子提交，REVIEW-R1-FIX: ISSUE-301）**+ **`_prepare_pool_info` 补 end_day None 守卫（worst_impact.py:390 `pool_duration_days = pe.end_day - pe.start_day`，Ph3 展平视图下永久 Banner 展平为 end_day=None、`None - int` 抛 TypeError，`end_day is None` 时兜底 21——REVIEW-R1-FIX: ISSUE-002，Ph3）**| ~20行变更 |
| `generator/schedule_generator.py` | **无需迁移**——经核实 :32-36 仅构造 `PoolSchedule`（core/schedule.py，字段保留，不受 Ph1 删 Pool 字段影响），无 `Pool(...)` 构造（REVIEW-R1-FIX: ISSUE-306，从波及范围移除）| 0 |
| `core/retreat_config.py` | `PoolEntry` 重建写侧改不传 `pool_type`/`rerun_of`（retreat_config.py:50/54 现以已删字段为关键字构造，ISSUE-007）+ 读侧停止读 `pool_type`（ISSUE-008）+ **`truncated.pools` 池重建写侧迁移到 `truncated.banner.banners`（BannerEntry/BannerPoolEntry，:39/:42，store.pools 写侧第 4 处，Ph3 属性化后 AttributeError/静默丢池，D3 写侧清单补充，REVIEW-R1-FIX: ISSUE-101）**+ **`offset_day` 与重建写侧 end_day 补 None 守卫（retreat_config.py:23 `from_pool.end_day > 0` / :47 `p.end_day - offset_day`，退避点或重建池为永久 Banner 时 `None > 0`、`None - int` 抛 TypeError，`end_day` 为 None 时兜底 `(start_day + 21)` 或保持 None——REVIEW-R1-FIX: ISSUE-002，Ph3）**| ~20行变更 |
...
| `core/gdr.py` | `compute_gdr_from_compact` 以 banner 聚合 + `pool_types` 消费（ISSUE-004）+ `filter_target_specs_by_obtainable` 改读 `store.banner`（ISSUE-009；**比较单位由天修正为秒——晚开池早停/截断场景可达性收严，REVIEW-R1-FIX: ISSUE-307**）| ~30行 |
| `core/process_trace.py` | `pool_types` 消费键对齐 + banner 聚合（ISSUE-004）| ~10行变更 |
| `core/retreat_search.py` <!-- REVIEW-FIX-PREV: AUDIT-BREAK-5 --> | `env.pools` 运行时消费端迁移（AUDIT-BREAK-5，Ph6）——`_get_obtainable_card_ids`（:407-412 读 `pool.is_exchange`/`pool.exchange_card_id`/`pool.rewards`）及多处 `if not env.pools`/`for pool in env.pools` 改按 banner 池展开读；**另补列模块级 `get_cost_per_draw`（:718-738，被 `PlanSearchEngine._extract_cost_per_draw` :219 调用）——`getattr(p, 'cost', None)` 在 Banner 上返回 None 静默回退 160，改为 `banner.active_pool.cost` 或展平读取（REVIEW-R1-FIX: ISSUE-304）** | ~14行变更 |
...
| `gui/main_window.py` / `gui/data_manager_panel.py` | `pool_types` 消费 + 可比性指纹 banner 维度（ISSUE-004/013）+ **`main_window.py:276-277` 内联分布提示删除/改写**（模板移除后 `not p.distribution_template` 恒 True、提示恒显示，REVIEW-R1-FIX: ISSUE-005，Ph8c）| ~20行变更 |
| `gui/process_analysis_panel.py` / `gui/analysis_panel.py` / `gui/data_manager_panel.py` / `gui/main_window.py`（pool_end_times 展示链） | **`env.pool_end_times` 键语义变化迁移（Ph6+Ph7，REVIEW-R1-FIX: ISSUE-006）**——`pool_end_times` 键由裸 pool_id 改 banner_id（batch_simulator.py:677 时间源改 Banner 级）后，`main_window.py:339/350` 转发、`process_analysis_panel.py:291-299`、`analysis_panel.py:1260-1261/1336-1346` 池结束时间图表（`sorted(self.pool_end_times.items())`）、`data_manager_panel.py:396` 数据集详情拿到 banner 级键——展示层按 banner 键或全限定键拆分展示（与 §3.5 要点 6 `banner_end_times_sorted` 同键语义） | ~8行变更 |
| `gui/analysis_panel.py` / `gacha_panel.py` / `resource_search_panel.py` / `worst_impact_panel.py` / `retreat_panel.py` / `cli.py` <!-- REVIEW-FIX-PREV: AUDIT-BREAK-5 --> <!-- REVIEW-FIX-PREV: AUDIT-BREAK-6 --> | `store.pools` 消费在 banner 模式经 Ph3 属性化只读视图保持非空（D3 裁决），消费方零改动（ISSUE-007 / REVIEW-R1-FIX: ISSUE-006）；**四处例外（其中 cli.py 两项均非「列表非空性」可覆盖，须随对应阶段迁移）——① `resource_search_panel.py:61` 消费 `env.pools`（运行时），Ph6 后元素为 `List[Banner]`、`p.cost` AttributeError（AUDIT-BREAK-5，Ph6）；② `gacha_panel.py:111` 读 `pool_end_resources` 键，Ph7 字段改名后 `.get` 静默空 dict（AUDIT-BREAK-6，Ph7）；③ `cli.py:148-150` 兜底目标 id 从 `store.pools[0].pool_id` **值**推导——Ph3 展平后为全限定键、兜底生成 `{banner}.main_ssr` 与真实卡 id 失配、无 `[[targets]]` 时 GDR 静默 0%（REVIEW-R1-FIX: ISSUE-003，Ph3 改取 banner 段）；④ `cli.py:61-69` `--migrate` 入口依赖将删的 `[[pools]]` 解析与 `store._migrated_from_legacy`——旧格式经 `-c` 传入时静默 0 Banner/`end_time=0`/模拟 0 抽且 `--migrate` 打印误导信息（REVIEW-R1-FIX: ISSUE-004，Ph4 处置）**；**另补列（REVIEW-R1-FIX: ISSUE-323/324/325）——⑤ `analysis_panel.py:1129-1137`（`no_draw_pool_resources.get(pid, {})` 的 pid 为全限定 `{banner_id}.{pool_id}` 键、no_draw 路径 banner_end_resources 为 banner 级键恒 miss——`resource_remaining` 每池 GDR 基线缺失）与 `retreat_panel.py:96`（透传同字典）——Ph7 按 banner_id 段（`k.split('.')[0]`）取或按 banner 段展平基线（ISSUE-323）；⑥ `analysis_panel.py:209/:1052` `_get_pool_names`/`_strip_pid` 以展平视图 pool_id/name 构建图表标签——§3.9 展平视图补 name 来源（取 banner.name）后标签显示中文池名、banner 模式 label 按 `banner_id.pool_id` 拆分（ISSUE-324，Ph3）；⑦ `cli.py:77` `config_meta['num_pools'] = len(store.pools)` 在 Ph3 展平视图下由「池条目数」漂移为「跨 Banner 内层池总数」——补 `banner_count` 输出或标注语义（ISSUE-325，Ph3）；⑧ `cli.py:212-213` `cumulative_snapshots_pools` 键列表随 Ph7 键全限定化（裸 pool id → banner/全限定键）——展示口径登记（ISSUE-325，Ph7）**| 0~（依赖只读视图）+ ~15行 |
| `core/streaming.py` | 聚合提取器支持 banner 维度 + **`compact.get('pool_end_resources', {})` 读点改 `.get('banner_end_resources', {})`（:114-115/:265/:284/:525/:533——累积快照 pool_end_resource(s) 字段改名后 `.get` 静默拿空 dict、`compute_gdr_from_cumulative` 回退 initial_resources/derive 推算分支、per-pool 累计 GDR 静默退化；键随 `pool_end_times` 同步改 banner 级，与 gdr.py:1115 消费链数据源一致，REVIEW-R1-FIX: ISSUE-322，Ph7）** | ~16行 |
| `tests/test_banner.py` | **新建**（含策略迁移回归，ISSUE-001）| ~200行 |
| `tests/core/test_state.py` <!-- REVIEW-FIX-PREV: AUDIT-BREAK-7 --> | 删除 `test_get_available_pools`（随 Ph1b 并入原子单元，REVIEW-R1-FIX: GATE-6_测试策略，ISSUE-002）| -12行 |
| `tests/service/test_gacha_service.py` <!-- REVIEW-FIX-PREV: AUDIT-BREAK-7 --> | 改写 `test_env_builder_from_config_store_smoke` 的 `pool_type` 断言 → 推导属性（随 Ph1b，REVIEW-R1-FIX: GATE-3_依赖顺序）+ **`_make_pool`（:14-21）与 `test_initial_count_multiple_cards`（:68-69）删 `available_from`/`available_until` 关键字（随 Ph1）；:142 `PoolEntry(pool_type='角色')` 移除关键字（随 Ph1b，AUDIT-BREAK-2/7）；:137 同测试的 `store.pools = [...]` 赋值改从 `store.banner` 构建（随 {Ph3, Ph4} 属性化，AUDIT-BREAK-3）**| -3行 |
| `tests/core/test_pool.py` <!-- REVIEW-FIX-PREV: AUDIT-BREAK-7 --> | `test_availability`（:36-41）以将删字段构造 `Pool` + 直测退役 `is_available_at`——删用例或改断言推导属性（随 Ph1/Ph1b）；**另 :24 `test_exchange_pool_returns_first_reward` 的 `Pool(... is_exchange=True)` 关键字移除（Ph1 将 is_exchange 改推导 property 后 TypeError——rewards=[(r, 1.0)] 单卡 100% 推导 `is_exchange=True` 语义不变，AUDIT-BREAK-3）** | -7行 |
| `tests/core/test_epitomizable_cards.py` <!-- REVIEW-FIX-PREV: AUDIT-BREAK-7 --> | :189/202-205 的 `Pool(pool_type=..., available_from=0, available_until=21)` 关键字移除（随 Ph1）| -4行 |
| `tests/core/test_pity_integration.py` <!-- REVIEW-FIX-PREV: AUDIT-BREAK-7 --> | :97/150 的 `Pool(pool_type='角色')` 关键字移除（随 Ph1）| -2行 |
| `tests/core/test_batch_draw.py` <!-- REVIEW-FIX-PREV: AUDIT-BREAK-7 --> | :24 的 `PoolEntry(pool_type='角色')` 关键字移除（随 Ph4）| -1行 |
| `tests/core/test_batch_draw.py`（策略部分）<!-- REVIEW-R1-FIX: AUDIT-BREAK-2 --> | :122 SmartStrategy 测试的 `StrategyContext(current_pools=[pool])` 直接构造（banners 默认值兜底）与 :130 断言 `len(call_args) >= 1` 改传 `banners=[Banner(id=..., pools={'main': pool})]` 包装——Ph1a 后 SmartStrategy 读 ctx.banners，不包装则恒空、call_args 空断言必失败（随原子单元，AUDIT-BREAK-2）| +3行 |
| `tests/core/test_strategy.py` <!-- REVIEW-R1-FIX: AUDIT-BREAK-2 --> | `_make_ctx`（:11 `StrategyContext(current_pools=...)` 直接构造）与 `test_target_hunting_strategy`（断言 `action.pool_id == 'standard'`）——改经 Banner 包装构造 ctx.banners；banners 字段默认值兜底避免 TypeError（随原子单元，AUDIT-BREAK-2）| +3行 |
| `tests/core/test_worst_impact_correctness.py` <!-- REVIEW-R1-FIX: AUDIT-BREAK-2 --> | 三个 draw_target 测试（:66/94/123，构造 :73/102/130）以 `_FakePool` 经 `current_pools=` 构造 + 断言 `action.pool_id` 裸键——改经 `Banner(id=..., pools={'main': ...})` 包装构造 ctx.banners、断言改 `action.banner_id`（随原子单元，AUDIT-BREAK-2）| +6行 |
| `tests/core/test_p69_strategy.py` <!-- REVIEW-R1-FIX: AUDIT-BREAK-2 --> | :223 `_min_strategy_context` 的 `build_strategy_context(**base)`——`banners`/`all_banners` 新参数须带默认值（否则 TypeError）；保持不传新参数可用（随原子单元 Ph5，AUDIT-BREAK-2）| 0（参数默认值） |
| `tests/core/test_gdr.py` <!-- REVIEW-R1-FIX: AUDIT-BREAK-3 --> | `_make_store_for_p62`（:604，:618 `store.pools = [...]`）改从 `store.banner`（BannerEntry available_from/available_until/enabled）构建——Ph3 属性化后赋值 AttributeError，且 store.banner 空时 P62 可达过滤目标卡整体误判不可达、`_obtainable` GDR 分母退化（随 {Ph3, Ph4}，AUDIT-BREAK-3）| ~4行 |
| `tests/core/test_retreat_config.py` <!-- REVIEW-R1-FIX: AUDIT-BREAK-3 --> | `_make_store_with_3_pools`（:11 `store.pools = [...]`）改从 `store.banner` 构建（随 {Ph3, Ph4}，AUDIT-BREAK-3）| ~3行 |
...
**文档同步（REVIEW-R1 新增——原计划零登记）：** <!-- REVIEW-FIX-PREV: ISSUE-011 -->
...
第一步：Ph0（core/notifier.py + gacha_service emit）—— ~30 行，半天
    ...
    ├── P61 Ph1-9（Banner 全套）
    │       依赖：无。Ph0 交付后即可启。
    │       前置：Ph1-base 基线固化（旧代码 + [[pools]] 配置跑固定种子，固化 golden fixture）
    ...
    │         （例外：{Ph3, Ph4} 同一交付单元——Ph3 将 store.pools 属性化只读、Ph4 才移除 _build_pools
    │         写侧，中间态 load_toml 对派生列表 append 静默丢池；**config_panel 写侧（:3879/:3385/:3925）
    │         与 config_toml 写侧同 commit 约束，{Ph3, Ph4} 一并迁移到 store.banner（AUDIT-BREAK-1，从 Ph8c
    │         前移），否则 Ph3 至 Ph8c 间 GUI 保存路径 AttributeError/静默丢池**，REVIEW-R1-FIX: ISSUE-311，
    │         见 §3.12 交付单元注）
    │
    └── P58 M1-8（Milestone 全套）
            依赖：无。Ph0 交付后即可启。
...
def register_milestone_engine(notifier, engine):
    ...
    # engine 来源（REVIEW-R1-FIX: ISSUE-329——跨进程传输契约）：worker 进程内经
    #   env.milestone_engine（SimulationEnv 新增字段，带默认值 None 保 pickle 兼容）取出，
    #   _run_single 在构造 GachaService 前注入；Windows spawn 下模块级全局 _milestone_engine
    #   重置为 None，不得依赖模块全局跨进程传递（batch_simulator._wk_init 仅 pickle env）。
    #   P58 未实施期 engine=None 时 _on_after_draw 内 `if _milestone_engine` 守卫跳过。
...
| 风险 | 缓解 |
|------|------|
| `current_pools` → `banners` 导致现有 8 种策略全部需要适配 | 策略迁移由 **Ph1a 强制完成**（与 Ph1/Ph1b/Ph1c/Ph2/Ph5 同 commit——首提交原子单元 {Ph1, Ph1a, Ph1b, Ph1c, Ph2, Ph5} 一次落地，REVIEW-R1-FIX: GATE-3，见 §3.12 交付单元注；避免中间态 AttributeError/TypeError）；`StrategyContext` 保留 `current_pools` 仅供兼容读（由 `active_banners[*].active_pool` 推导填充），策略层以 `banners` 为准。`current_pools` 元素在 **Ph6 多池 banner 模式**下 `.id` 为 pools 字典键（`'main'`，非全局唯一）——fixed_count / target_hunting 依赖池 id 匹配、裸 `'main'` 反查歧义，一并纳入强制迁移（REVIEW-R1-FIX: ISSUE-001/003/321；原子提交态下其 `.id` 为旧裸池 id，迁移理由以 Ph6 后口径为准） |
...
| Banner 内部 pool 切换逻辑与 PityEngine 的交互复杂（送抽pool排除保底时，保底计数器跨pool是否继承） | Banner.draw() 内部显式分两路：`excludes_all_pity` 的 pool 完全旁路 `before_draw/after_draw`，不触碰 `pity_state`，**且 DrawOutcome.pity_triggered 恒 False、不查询 spec/featured_ids（与 §3.8『pity_triggers 仅统计非 excludes_all_pity』一致，REVIEW-R1-FIX: ISSUE-320）**；正常 pool 走完整保底管线 |
| 生命周期规则遗漏边界条件（如送抽pool激活时主pool恰好也被时间窗口过期关闭） | 规则评估顺序：时间窗口（最先——**`is_available(state.real_time)` 时间比较 + on_banner_end 关闭标记双路径**，过期直接 exhaust banner，REVIEW-R1-FIX: ISSUE-001）→ 转换条件 → 可用性汇总。时间过期的 banner 不进入 `active_banners`（is_available 时间比较排除，§3.2 / §3.5 伪代码）；窗口未开 banner 同样被排除 |
| `[[banner]]` TOML 解析复杂度——pool / lifecycle 嵌套段与现有 `[[pools]]` 格式差异大 | `config_toml.py` 中检测到 `[[banner]]` 段时走新解析路径；D3/D4 一次性迁移后 config.toml 无 `[[pools]]` 段残留、仅 `[[banner]]`（段名以现状 `[[pools]]` 为准，REVIEW-R1-FIX: ISSUE-308） |
| 现有 `Pool` 对象缓存和共享引用在无自动包装后如何构造 | 一次性迁移后 Pool 直接进 `[[banner.pool]]`，构造点按新签名构造 `Banner`（§3.4 / Ph1a 构造点迁移）；无包装层。`generator/schedule_generator.py` 仅构造 `PoolSchedule`（字段保留、无需迁移），不属本行范围（REVIEW-R1-FIX: ISSUE-306） |
...
| 活跃池耗尽且无 lifecycle 接管（单池 Banner 只配一个池）时 draw() 行为未定义 | 兜底语义：Banner 自动 exhaust（§3.2）——`is_available(real_time)=False`、`is_exhausted=True`；空 rewards 推导 random 显式短路为 False（§3.13.1），避免 IndexError（REVIEW-R1-FIX: ISSUE-010） |
...
| `store.pools` 属性化只读后 `core/retreat_config.py` 的 `truncated.pools` 池重建写侧（:39/:42，`RetreatConfigBuilder.build`）对无 setter property 赋值 AttributeError / 对派生列表 append 静默丢池，retreat_search.py:134 截断时间线模式崩溃或拿到空池配置（REVIEW-R1-FIX: ISSUE-101） | 写侧随 Ph3 迁移到 `truncated.banner.banners`（BannerEntry/BannerPoolEntry），`from_pool_id` 定位适配全限定键（§3.9 / §3.12 Ph3） |
| `Pool(...)` 关键字构造调用方（config_service/worst_impact/retreat_config；schedule_generator 经核实仅构造 `PoolSchedule`、无需迁移，REVIEW-R1-FIX: ISSUE-306）在删字段后 TypeError | Ph1a 构造点迁移：删字段参数、时间窗口移 Banner 级、is_exchange 改 property、停止读 pool_type；worst_impact 的 99 池错峰窗口改直接构造带 `available_from`/`available_until` 的 Banner（REVIEW-R1-FIX: ISSUE-302）；retreat_config 的 `PoolEntry` 重建同时改不传 pool_type/rerun_of（写侧，ISSUE-007/008） |
| 多 Banner 各含同名 `main` 时 PityEngine 裸 `pool.id` 键冲突、保底串池 | 全限定键 `{banner_id}.{pool_id}` 统一键空间（§3.11.3 / ISSUE-010，Ph2/Ph6） |
| 旧 `PityDef.pools` 裸池 id fnmatch 模式（`pools = ["genshin_limited"]`）在全限定键（`genshin_limited.main`）下失绑——fnmatch 全串匹配命中不了，保底规则静默不作用于该池 | **PityDef.pools 改全限定键随原子提交落地**（`{banner_id}.{pool_id}`，从 Ph4b 前移，避免原子提交态非 `'*'` 裸 id 绑定失配，REVIEW-R1-FIX: ISSUE-103）；Ph9 用迁移后用例锁定绑定行为（§3.11.3 / REVIEW-R1-FIX: ISSUE-004，Ph8b） |
| 保存侧只写 `[[pools]]`，banner 配置无法 round-trip；pool_type/rerun_of 解析残留 | 保存侧统一写 `[[banner]]` + pool_type/rerun_of 残留清理（§3.9 / ISSUE-012，D3/D4 后简化，Ph4） |
...
| 原子提交→Ph6 间构造桥 None 窗口、逐池时间窗口抽取过滤失效：错峰池在窗口外被抽取，抽数分布/目标卡/GDR 改变，与旧模型 `current_pools` 过滤语义分歧（REVIEW-R1-FIX: ISSUE-102） | 显式承认为已知中间态分歧并收缩 GATE-3 验收范围（「非时间窗口行为不变」）；Ph4b golden 等价对照改排 Ph6 后；逐池窗口过滤等价性由 Ph6 后恢复窗口的 Ph1-base golden 对照锁定（§3.5 要点 10 / §3.12 Ph4b / 交付单元注） |
...
| worst_impact 独立 `_build_pity_engine`（:443-510）pool_specs 裸 `_worst_impact_pool_N` 键不在全限定键迁移清单内——Ph2 后 gacha_service 以 `{banner.id}.{pools 字典键}` 查询、`pool_specs.get(pool_id)` 恒 None、最差影响分析保底静默失效，且最差影响走 from_dict 直构不经 Ph6 from_config_store 迁移（REVIEW-R1-FIX: ISSUE-301） | 在 §3.11.3 全限定键迁移清单显式枚举 worst_impact 构建器：pool_specs 键改 `{pid}.main`（单池包装口径）、pdef.pools 的 fnmatch 同按全限定键匹配，随原子提交落地而非 Ph6；Ph9 增加最差影响分析保底触发回归用例（§3.11.3 / §3.12 Ph1a+Ph9） |
| worst_impact 99 池错峰时间窗口在 Ph1a 删字段后永久丢失（from_dict 直构 List[Pool] 不经 Ph6 修复）——构造桥包装 Banner 窗口 None → 99 池恒可用、ConsecutivePoolTargetCondition 提前停机、expected_pools 恒≈1，且与「Ph6 后恢复窗口」处置前提矛盾（REVIEW-R1-FIX: ISSUE-302） | Ph1a 改为 from_dict 直构带 `available_from`/`available_until` 的 Banner（Banner 级窗口、构造桥对 Banner 元素直接收纳，原子提交态即生效）；Ph9 增加 `analyze()` 级回归锁定 `expected_pools` 不退化（§3.12 Ph1a / §3.5 要点 10 例外注 / Ph9） |
| 4 策略 `_pool_needs_target(pool.id, ctx)`（smart/pity_reserve/pool_quota/stop_on_target）以裸 `pool.id in t.pool_ids` 匹配——原子提交态裸 id 恰好命中（掩盖冲突），Ph3 后 `card_defs.pools` 变全限定键、Ph6 后 `pool.id` 变 pools 字典键，裸匹配静默失效、策略只等待不抽卡（REVIEW-R1-FIX: ISSUE-303/315/321） | Ph1a 规定匹配参数改 `banner.id`（原子提交态 banner.id == 旧裸池 id、Ph6 后 == Banner id），`TargetCard.pool_ids` 钉死为 banner 级键（batch_simulator 取全限定键 banner_id 段，ISSUE-315）；Ph9 增加 4 策略 banner 模式目标追卡回归（§3.4 / §3.12 Ph1a+Ph9） |
| `retreat_search.get_cost_per_draw`（:718-738）`getattr(p, 'cost', None)` 防御式读取在 env.pools 改 `List[Banner]` 后静默回退 160（非崩溃型、不在 AUDIT-BREAK-5 必然 AttributeError 清单内，实施易漏改）——`_extract_cost_per_draw` 的 `epsilon = cost_per_draw * precision_draws` 二分收敛阈值与 `result.cost_per_draw` 报告对成本≠160 配置静默失真（REVIEW-R1-FIX: ISSUE-304） | Ph6 AUDIT-BREAK-5 清单补列 get_cost_per_draw（改 `banner.active_pool.cost` 或展平读取）；Ph9 增加非 160 成本配置的 `cost_per_draw` 回归断言（§3.12 Ph6+Ph9 / §四 波及范围） |
| Ph5「删除 current_pools/all_pools」与 §3.4/§3.5 要点 8/§六/§七「保留」自相矛盾——删除分支下 `get_pity_probabilities`（strategy.py:109-121）遍历 current_pools 必断、test_p69_strategy.py:197-204/223 以 `current_pools=[]` 构造必 TypeError 且不在 AUDIT-BREAK-7 清单（REVIEW-R1-FIX: ISSUE-305） | 统一为「保留 current_pools/all_pools 仅遗留只读（由 `active_banners[*].active_pool` 推导填充，不删除）」+ `get_pity_probabilities` 改经 banners 维度定位全限定键，同步修订 §3.12 Ph5 与 §六/§七 冲突表述（§3.12 Ph5） |
| `filter_target_specs_by_obtainable` 比较单位由天修正为秒——晚开池（如样本 pool_c8 day 147）在早停/截断时间线由「可达」变「不可达」、4 个 `_obtainable` GDR 分母变化，§3.13.4 未声明该行为变化（REVIEW-R1-FIX: ISSUE-307） | §3.13.4 显式声明比较单位修正为秒的行为影响（晚开池可达性收严），确认符合 P62 可达过滤语义（有意修正）；§七 验收措辞相应更新；Ph9 补充晚开池早停场景 `_obtainable` GDR 用例（§3.13.4 / §七 / Ph9） |
| CLI 无 `[[targets]]` 时兜底目标 id 从 `store.pools[0].pool_id` **值**推导——Ph3 展平后为全限定 `{banner_id}.{pool_id}`，兜底生成 `{banner}.main_ssr` 与任何真实卡 id 失配、ssr 计数恒 0、CLI 核心指标 GDR 静默 0%（REVIEW-R1-FIX: ISSUE-003） | Ph3 改取 banner 段 `store.pools[0].pool_id.split('.')[0] + '_ssr'` 或改从 `store.banner` 推导兜底目标（§3.9 / §3.12 Ph3）；Ph9 增加 CLI 无 `[[targets]]` 配置下 GDR 非 0% 回归 |
| CLI `--migrate` 依赖将删的 `[[pools]]` 解析与 `store._migrated_from_legacy`——旧格式经 `-c` 传入时 `_migrated_from_legacy` 恒 False、打印「无需迁移」误导信息且不做迁移；非 migrate 路径空 banner → `end_time = max(...) if banners else 0` = 0 → `AllPoolsEndCondition(end_time=0)` 立即成立 → 模拟 0 抽、CLI 静默输出全 0（REVIEW-R1-FIX: ISSUE-004） | Ph4 删除/废弃 `--migrate` 入口，或空 banner 时显式报错提示先手工迁移；`from_config_store`/`load_toml` 在 banner 为空且检测到旧 `[[pools]]` 段残留时给显式警告而非静默 end_time=0（§3.12 Ph4 / §四 波及范围） |
| config_panel 写侧中间态：Ph3 将 `store.pools` 属性化只读后，`config_panel.py:3879`（apply_to_store `store.pools = []`）对无 setter property 赋值 AttributeError、`:3385/:3925`（set_config/apply_to_store append）对派生列表 append 静默丢池——原计划仅 ISSUE-311 协调 config_toml 写侧、config_panel 写侧无任何处置，Ph3 至 Ph8c 间 GUI 保存路径崩溃，与「任何阶段系统可运行」冲突（AUDIT-BREAK-1） | config_panel 写侧迁移随 {Ph3, Ph4} 同 commit 落地（从 Ph8c 前移，写 `store.banner` BannerEntry/BannerPoolEntry；或 ISSUE-311 第二选项临时 `store._pools` 内部可变字段）；Ph8c 仅保留 Tab UI 重写与读侧 Banner 适配（§3.9 写侧段 / §3.12 Ph3+Ph8c） |
| `StrategyContext.banners`/`all_banners` 无默认值 → 直接构造测试（test_worst_impact_correctness:73/102/130、test_strategy:11、test_batch_draw:122）与 test_p69_strategy:223 `build_strategy_context(**base)` TypeError；且 Ph1a 迁移后这些测试的策略经 `ctx.banners` 读取、不传 banners 则恒空列表 → draw_target/target_hunting/SmartStrategy 断言失败（AUDIT-BREAK-2，4a transitive_gaps 独立验证） | `banners`/`all_banners` 字段与 `build_strategy_context` 新参数带默认值（`field(default_factory=list)` / `banners=None` 置空）；四个测试文件随原子单元改经 Banner 包装构造 `ctx.banners`（§3.12 Ph1a+Ph5 / §3.12 交付单元注 ⑥-⑨ / §四 波及范围） |
...
| **DECISION-4** | 兼容模式统计键格式（§3.13.1） | **一次性迁移到全限定键 `{banner_id}.{pool_id}`**，不保留旧裸键、不做旧数据迁移映射（砍兼容） | 全限定 + 旧数据集键迁移映射 | 新代码仅一种键格式；无历史结果（未上线），指纹版本化直接按 banner 维度；「兼容」仅指旧 `[[pools]]` 配置允许一次性改写为 `[[banner]]`（等价性由基线对照验证） | §3.13.1 / §3.11.3 / gdr / process_trace / main_window / data_manager | **Ph7 启动前（已裁决）** |

**裁决状态（2026-08-02 用户确认，依据「无历史包袱」原则——见 CLAUDE.md）**：四项全部按一次性迁移方向裁决，门控已关闭。**决策依据**：这些分叉源于「旧 `[[pools]]` 必须向后兼容」的假设；本项目未上线、config.toml 仅为示例与测试用文件（CLAUDE.md「无历史包袱原则」），该假设不成立，故全部简化为一次性迁移；等价性由「基线固化 + 等价对照」保障（见 §3.12 Ph1 基线阶段 / §七 验收）。**改判代价**：D1 在 Ph1 后改判需恢复 one_shot 字段并重写耗尽判定；D3 在 Ph4 后改判需恢复 dual-write 或迁移 15 处消费点；D4 在 Ph7 后改判需恢复键兼容读取。

<!-- REVIEW-FIX-PREV: GATE-5 -->
### 6.2 回滚策略
...
**引擎与集成：**
- [ ] 配置一次性迁移完成：`[[banner]]` 解析正确、旧 `[[pools]]` 配置已改写（无自动包装路径），迁移后模拟结果与迁移前逐字段一致（基线固化等价对照，见基线阶段；**对照以 Ph6 后 from_config_store 恢复窗口的运行结果为准**，Ph6 前构造桥 None 窗口、逐池窗口过滤失效，此间重跑结果与 golden 的差异属中间态分歧而非迁移行为差异，REVIEW-R1-FIX: ISSUE-102）
- [ ] CLI 无 `[[targets]]` 时兜底目标 id 在 banner 模式取 banner 段（`store.pools[0].pool_id.split('.')[0]`）——GDR 不静默 0%（ssr 计数不恒 0，REVIEW-R1-FIX: ISSUE-003，Ph3/Ph9）
- [ ] CLI `-o` 输出元数据可比性——`num_pools` 在展平视图下标注「跨 Banner 内层池总数」（或补 `banner_count` 输出），`cumulative_snapshots_pools` 键列表随 Ph7 键全限定化后展示口径明确，不静默改变 CLI 输出语义（REVIEW-R1-FIX: ISSUE-325，Ph3/Ph7）
- [ ] CLI `--migrate` 入口已处置——旧 `[[pools]]` 格式经 `-c` 传入时不再打印「已为新格式」误导信息（删除/废弃入口或显式报错提示先手工迁移）；空 banner 时 from_config_store/load_toml 显式警告而非静默 `end_time=0`、模拟 0 抽（REVIEW-R1-FIX: ISSUE-004，Ph4/Ph9）
...
- [ ] `banner.pending_transitions(real_time)` 正确暴露「还差X抽触发Y」——**带参方法（时间求值输入经入参提供，与 `is_available(real_time)` 对称，策略经 `ctx.state.real_time` 传入，REVIEW-R1-FIX: ISSUE-330）**；事件型条件（card_obtained / pool_exhausted）remaining=-1（不可预估）、阈值型（pool_draws/banner_draws/time_window）为剩余抽数/向上取整剩余天数（**time_window 的剩余天 = 秒差 `/ DAY` 后向上取整**——2.5 天→3、单位与 ISSUE-001 秒统一，REVIEW-R1-FIX: ISSUE-305/317）；策略示例带 `pt.remaining >= 0` 守卫，事件型规则不被误判为「立刻可触发」，time_window 2.5 天不截断为 2（REVIEW-R1-FIX: ISSUE-305）
...
- [ ] `StrategyContext` 同时提供 `banners` 和 `current_pools`（后者由 `active_banners[*].active_pool` 推导填充）；8 个内置策略经 Ph1a 迁移后，banner 模式与旧 `[[pools]]` 模式运行结果等价（策略迁移是强制项——「无需修改」验收取消）（REVIEW-R1-FIX: ISSUE-001）
- [ ] fixed_count / target_hunting 已随 Ph1a 迁移到 `ctx.banners` 双字段——target_hunting 的 `target_pool_ids` 匹配 `banner_id`（Ph6 多池 banner 模式 `active_pool.id` 为 pools 字典键、不再用其裸 id 匹配，目标池命中 Banner id、不恒空）；fixed_count 的 `DrawAction` 带 `banner_id`，不依赖 `current_pools` 元素的池 id 反查（REVIEW-R1-FIX: ISSUE-003/321）
- [ ] smart / pity_reserve / pool_quota / stop_on_target 4 策略的 `_pool_needs_target` 以 `banner.id` 匹配 `TargetCard.pool_ids`——banner 模式下目标卡未达成时策略实际抽卡而非只等待（REVIEW-R1-FIX: ISSUE-303）
- [ ] draw_target 的 `pool_id` 参数改匹配 `banner.id`——非空 pool_id 配置在 banner 模式下命中对应 Banner、不恒 WaitAction（worst_impact 唯一消费方不退化）；`pool_id=''` 保持「匹配任意」（REVIEW-R1-FIX: ISSUE-326，Ph1a/Ph9）
- [ ] pool_quota 的 `pool_quotas` 参数键为全限定 `{banner_id}.{pool_id}`——多 Banner 各含同名 'main' 时配额不串池（单 Banner 单池裸键兼容保留），Ph9 用例锁定（REVIEW-R1-FIX: ISSUE-327，Ph1a/Ph9）
- [ ] worst_impact 最差影响分析保底不静默失效——`_build_pity_engine` pool_specs 键为全限定 `{pid}.main`、与 gacha_service 查询键同口径，`before_draw` 保底调整生效（REVIEW-R1-FIX: ISSUE-301）
- [ ] worst_impact 最差影响分析 99 池错峰窗口保留——`analyze()` 结果 `expected_pools ≈ MAX_POOLS` 不退化（连续池目标可达成，REVIEW-R1-FIX: ISSUE-302）
- [ ] `get_cost_per_draw` 在 `env.pools` 承载 `List[Banner]` 后返回实际池成本（`banner.active_pool.cost`）而非静默回退 160——非 160 成本配置（如 orundum:600）的二分收敛与 `result.cost_per_draw` 报告不失真（REVIEW-R1-FIX: ISSUE-304）
...
- [ ] `GachaService.__init__` 构造桥生效——`_run_single`（batch_simulator.py:228）与现有测试直接构造 `GachaService([pool],...)` 后 `self._banners` 非空、`run_simulation` 正常路由；`from_config_store`（batch_simulator.py:547-559）不再以已删字段（`available_from`/`available_until`/`pool_type`/`is_exchange`）构造 `Pool(...)`（REVIEW-R1-FIX: GATE-3_依赖顺序，Ph1a + Ph2）；**构造桥窗口语义：Ph6 前包装 Banner 窗口为 None（原子提交→Ph6 间逐池窗口过滤不生效，为已知中间态分歧，GATE-3 验收收缩为「非时间窗口行为不变」），Ph6 后 from_config_store 从 store.banner 恢复窗口、逐池窗口过滤等价性经 Ph1-base golden 对照锁定（REVIEW-R1-FIX: ISSUE-102）**
- [ ] 时间窗口可用性过滤引擎落点——`active_banners` 过滤显式传 `state.real_time` 调 `banner.is_available(real_time)`：窗口未开（`real_time < available_from`）与已关（`real_time > available_until`）的 Banner 不进入 active_banners、策略不可再抽（与现状 gacha_service.py:224-226 逐池 real_time 过滤语义等价）；`on_banner_end` 触发（`real_time >= available_until`）时兼作 Banner 关闭标记（调用 `_exhaust()`、`is_exhausted=True`）；窗口未开 Banner 等待期到 available_from 后重新可抽（REVIEW-R1-FIX: ISSUE-001，Ph9 用例锁定）
- [ ] 池结束快照机制已迁移：`on_banner_end` 写 `CompactResult.banner_end_resources`/`banner_end_pity_states`，P62 可达过滤 / per_pool_analysis / `core/vulnerability.py` 逐池分箱消费端一致——脆弱性分析字段改名后不静默返回空结果（REVIEW-R1-FIX: ISSUE-003/005）；**`core/streaming.py` 生产端读点（:114-115/:265/:284/:525/:533）改 `.get('banner_end_resources', {})` 且键随 `pool_end_times` 改 banner 级——`compute_gdr_from_cumulative` 不静默回退推算分支（REVIEW-R1-FIX: ISSUE-322）；no_draw 路径 banner_end_resources 键为 banner 级，`analysis_panel.py:1129-1137` / `vulnerability.py:941-942` 按 banner_id 段取基线，`resource_remaining` 每池 GDR 基线不静默缺失（REVIEW-R1-FIX: ISSUE-323，Ph9 用例锁定）**
- [ ] `env.pool_end_times` 键语义变化已登记——GUI 展示链（`main_window.py:339/350` 转发、`process_analysis_panel.update_results(pool_end_times=...)`、`analysis_panel.py:1260-1261/1336-1346` 池结束时间图表、`data_manager_panel.py:396` 数据集详情）拿到 banner 级键后按 banner 键或全限定键拆分展示，逐池累计分析/展示语义不静默改变（REVIEW-R1-FIX: ISSUE-006，Ph6/Ph7）
...
- [ ] `filter_target_specs_by_obtainable` 读 `store.banner`——banner 模式下 `_obtainable` GDR 分母与 `[[pools]]` 模式在全时间线配置下语义一致；**晚开池在早停/截断时间线的可达性由「恒可达」修正为「不可达」（比较单位天→秒的有意修正，符合 P62 可达过滤语义，分母随之收严）**（REVIEW-R1-FIX: ISSUE-009/307）
...
- [ ] `SimulationEnv.milestone_engine=None` 字段带默认值——跨进程 pickle 兼容、`_run_single` 装配前可从 `env.milestone_engine` 取出注入 `register_milestone_engine`，Windows spawn 下不依赖模块级全局（REVIEW-R1-FIX: ISSUE-329，Ph0 契约验收）
...
- [ ] `store.pools` 属性化只读视图：从 `store.banner` 实时展平（`pool_id` 为全限定 `{banner_id}.{pool_id}`），旧字段读者不拿空列表；写侧已全部迁移到 `store.banner`（D3 裁决，无 dual-write，**含 `core/retreat_config.py` 的 `truncated.pools` 写侧迁移到 `truncated.banner.banners`：retreat_search 截断时间线模式不崩溃、不静默丢池**，REVIEW-R1-FIX: ISSUE-101；**含 config_panel 写侧 `config_panel.py:3879/:3385/:3925` 随 {Ph3, Ph4} 同 commit 迁移（AUDIT-BREAK-1）——属性化落地后 GUI 保存路径不抛 AttributeError、不静默丢池，与 config_toml `_build_pools` 写侧同 commit 约束，Ph8c 仅保留 Tab UI 重写与读侧 Banner 适配**）
- [ ] 展平视图 `PoolEntry.name` 取 `banner.name`——`analysis_panel._get_pool_names`/`_strip_pid` 图表标签显示中文池名而非原始全限定键，banner 模式 label 按 `banner_id.pool_id` 拆分（REVIEW-R1-FIX: ISSUE-324，Ph3/Ph9）
- [ ] 展平视图 end_day=None 守卫——永久 Banner（`available_until=None`）展平为 `end_day=None` 后，`core/worst_impact.py:390` `_prepare_pool_info`（`pe.end_day - pe.start_day`）与 `core/retreat_config.py:23/47`（`from_pool.end_day > 0` / `p.end_day - offset_day`）均不抛 TypeError——`end_day is None` 时兜底 21 / `start_day + 21`，重建写侧保持 None；配置含永久 Banner 的最差影响分析与退避分析正常（REVIEW-R1-FIX: ISSUE-002，Ph9 用例锁定）
...
- [ ] 无自动包装路径（新建仅认 `[[banner]]` 的解析路径——现状代码库中无 `_wrap_pools_as_banners` 函数可删，REVIEW-R1-FIX: ISSUE-308）；config.toml 一次性迁移完成（D3/D4 裁决）
...
- [ ] 加载时：`pools` fnmatch pattern → 自动勾选匹配行；保存时：勾选行 → 自动生成紧凑 fnmatch pattern；**「全不选」（0 行勾选）保存为 `pools = []`（= 不绑定任何池，保底规则不生效），不沿用旧「空文本 → `('*',)`」逻辑（REVIEW-R1-FIX: ISSUE-328，Ph8b/Ph9 用例锁定）**
- [ ] `[全选]` `[全不选]` 按钮
- [ ] PityDef.pools 绑定为全限定键 fnmatch：`pools = ["genshin_limited.main"]` 精确命中；配置迁移已把旧裸 id（`genshin_limited` / `main`）改写为全限定键（**随原子提交落地**，REVIEW-R1-FIX: ISSUE-103 / ISSUE-004）
...
- [ ] `tests/core/test_pool.py` 的 `is_available_at` 用例已删除/改写——不再直测退役 API、不以将删字段构造 `Pool`（AUDIT-BREAK-7）；**`test_exchange_pool_returns_first_reward`（:24）不再以 `is_exchange=True` 关键字构造 `Pool`（推导 property 后 TypeError，rewards 单卡 100% 推导语义不变，AUDIT-BREAK-3）**；`tests/core/test_epitomizable_cards.py:189/202-205`、`tests/core/test_pity_integration.py:97/150` 不再以 `pool_type`/`available_from`/`available_until` 构造 `Pool`（AUDIT-BREAK-7）；`tests/core/test_batch_draw.py:24`、`tests/service/test_gacha_service.py:142` 不再以 `pool_type` 构造 `PoolEntry`（随 Ph4 删字段，AUDIT-BREAK-4/7）
- [ ] `StrategyContext.banners`/`all_banners` 字段与 `build_strategy_context` 新参数带默认值——策略层直接构造测试（`test_worst_impact_correctness.py:66/94/123` 构造 :73/102/130、`test_strategy.py:11`、`test_batch_draw.py:122`、`test_p69_strategy.py:223`）不因缺失 `banners=` 抛 TypeError；三个 draw_target 测试、`test_target_hunting_strategy`、SmartStrategy `:130` 断言均改经 Banner 包装的 `ctx.banners` 后行为正确（Ph1a 迁移后不恒 WaitAction、断言不失败）（随原子单元，AUDIT-BREAK-2）
- [ ] `tests/core/test_gdr.py:604 _make_store_for_p62`（:618）、`tests/core/test_retreat_config.py:11 _make_store_with_3_pools`、`tests/service/test_gacha_service.py:137` 的 `store.pools = [...]` 赋值已改从 `store.banner` 构建——Ph3 属性化后不抛 AttributeError、不静默丢池，P62 可达过滤 fixture 的 `_obtainable` GDR 分母不退化（随 {Ph3, Ph4}，AUDIT-BREAK-3）
- [ ] 原子提交态 pity 键一致——`_build_pity_engine_from_gui`/`from_config_store` 的 pool_specs 键为全限定 `{pool.id}.main` 且与 gacha_service 查询键同口径；**PityDef.pools 裸 id→全限定键改写随原子提交一并落地**（config.toml `[pity_defs]` 段，从 Ph4b 前移），非 `'*'` 裸 id 绑定在原子提交态即精确命中（`fnmatch('pool_c1.main', 'pool_c1.main')` = True）、soft pity 不静默失效（AUDIT-BREAK-3 / §3.5 要点 12 / REVIEW-R1-FIX: ISSUE-103）
...
<details>
<summary>第 2 次审查（2026-06-11）——P38 工作流自动化</summary>
